...
import (
	...
	vgclose "github.com/elysiumstation/fury/libs/close"
	vgjob "github.com/elysiumstation/fury/libs/job"
	...
	walletapi "github.com/elysiumstation/fury/wallet/api"
	"github.com/elysiumstation/fury/wallet/api/interactor"
	netStoreV1 "github.com/elysiumstation/fury/wallet/network/store/v1"
	"github.com/elysiumstation/fury/wallet/preferences"
	"github.com/elysiumstation/fury/wallet/service"
	svcStoreV1 "github.com/elysiumstation/fury/wallet/service/store/v1"
	serviceV1 "github.com/elysiumstation/fury/wallet/service/v1"
	serviceV2 "github.com/elysiumstation/fury/wallet/service/v2"
	"github.com/elysiumstation/fury/wallet/service/v2/connections"
	"github.com/elysiumstation/fury/wallet/service/v2/connections/store/longliving/v1"
	sessionStoreV1 "github.com/elysiumstation/fury/wallet/service/v2/connections/store/session/v1"
...
var (
	runServiceLong = cli.LongDesc(`
		Start a Fury wallet service behind an HTTP server.
...
func BuildCmdRunService(w io.Writer, handler RunServiceHandler, rf *RootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "run",
		Short:   "Start the Fury wallet service",
		Long:    runServiceLong,
...
func RunService(w io.Writer, rf *RootFlags, f *RunServiceFlags) error {
	...
	cliLog, cliLogPath, _, err := buildJSONFileLogger(furyPaths, paths.WalletCLILogsHome, "info")
	if err != nil {
		return err
	}
	...
	walletStore, err := wallets.InitialiseStoreFromPaths(furyPaths, true)
	if err != nil {
	...
	netStore, err := netStoreV1.InitialiseStore(furyPaths)
	if err != nil {
	...
	sessionStore, err := sessionStoreV1.InitialiseStore(furyPaths)
	if err != nil {
	...
	if f.LoadTokens {
		...
		s, err := v1.InitialiseStore(furyPaths, f.tokensPassphrase)
		if err != nil {
	...
	loggerBuilderFunc := func(levelName string) (*zap.Logger, zap.AtomicLevel, error) {
		svcLog, svcLogPath, level, err := buildJSONFileLogger(furyPaths, paths.WalletServiceLogsHome, levelName)
		if err != nil {
...
func buildJSONFileLogger(furyPaths paths.Paths, logDir paths.StatePath, logLevel string) (*zap.Logger, string, zap.AtomicLevel, error) {
	loggerConfig := vgzap.DefaultConfig()
	loggerConfig = vgzap.WithFileOutputForDedicatedProcess(loggerConfig, furyPaths.StatePathFor(logDir))
