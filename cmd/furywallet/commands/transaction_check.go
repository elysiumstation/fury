...
var (
	...
	checkTransactionExample = cli.Examples(`
		...
		# Check a transaction and send it to a specific Fury node address
...
func BuildCmdCheckTransaction(w io.Writer, handler CheckTransactionHandler, rf *RootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "check",
		Short:   "Check a transaction and send it to a Fury node",
	...
	cmd.Flags().StringVar(&f.NodeAddress,
		"node-address",
		"",
		"Fury node address to which the transaction is sent after it is checked",
	)
