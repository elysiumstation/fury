...
func (cmd *PostgresRunCmd) Execute(_ []string) error {
	...
	configWatcher, err := config.NewWatcher(ctx, log, furyPaths, config.Use(parseFlagOpt))
	if err != nil {
		return err
	}
	...
	dbConfig := embeddedpostgres.DefaultConfig().
		...
		RuntimePath(furyPaths.StatePathFor(paths.DataNodeStorageSQLStoreHome)).
		DataPath(furyPaths.StatePathFor(paths.DataNodeStorageSQLStoreNodeDataHome))
