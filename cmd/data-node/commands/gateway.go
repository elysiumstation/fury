...
func (opts *gatewayCmd) Execute(_ []string) error {
	...
	cfgwatchr, err := config.NewWatcher(ctx, log, furyPaths)
	if err != nil {
	...
	eg.Go(func() error {
		srv := server.New(opts.Config, log, furyPaths)
...
func Gateway(ctx context.Context, parser *flags.Parser) error {
	...
	_, err := parser.AddCommand("gateway", "The API gateway", "The gateway for all the fury APIs", opts)
	return err
}
