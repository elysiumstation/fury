...
func (cmd *StartCmd) Execute(args []string) error {
	...
	configWatcher, err := config.NewWatcher(context.Background(), log, furyPaths, config.Use(parseFlagOpt))
	if err != nil {
		return err
	}
...
func Start(_ context.Context, parser *flags.Parser) error {
	...
	cmd, err := parser.AddCommand("start", "Start a fury data node", "Start a fury data node as defined by the config files", &startCmd)
	if err != nil {
		return err
	}
