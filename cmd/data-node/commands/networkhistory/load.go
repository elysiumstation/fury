...
func (cmd *loadCmd) Execute(args []string) error {
	...
	hasSchema, err := sqlstore.HasFurySchema(ctx, connPool)
	if err != nil {
...
func createNetworkHistoryService(ctx context.Context, log *logging.Logger, furyConfig config.Config, connPool *pgxpool.Pool, furyPaths paths.Paths) (*networkhistory.Service, error) {
	snapshotService, err := snapshot.NewSnapshotService(log, furyConfig.NetworkHistory.Snapshot, connPool,
		furyPaths.StatePathFor(paths.DataNodeNetworkHistorySnapshotCopyTo), func(version int64) error {
			if err := sqlstore.MigrateUpToSchemaVersion(log, furyConfig.SQLStore, version, sqlstore.EmbedMigrations); err != nil {
				return fmt.Errorf("failed to migrate to schema version %d: %w", version, err)
			}
			return nil
		},
		func(version int64) error {
			if err := sqlstore.MigrateDownToSchemaVersion(log, furyConfig.SQLStore, version, sqlstore.EmbedMigrations); err != nil {
	...
	networkHistoryStore, err := store.New(ctx, log, furyConfig.ChainID, furyConfig.NetworkHistory.Store, furyPaths.StatePathFor(paths.DataNodeNetworkHistoryHome),
		furyConfig.MaxMemoryPercent)
	if err != nil {
		return nil, fmt.Errorf("failed to create network history store:%w", err)
	}

	networkHistoryService, err := networkhistory.NewWithStore(ctx, log, furyConfig.ChainID, furyConfig.NetworkHistory,
		connPool, snapshotService, networkHistoryStore, furyConfig.API.Port,
		furyPaths.StatePathFor(paths.DataNodeNetworkHistorySnapshotCopyTo))
	if err != nil {
