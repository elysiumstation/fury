...
func (l *NodeCommand) persistentPre([]string) (err error) {
	...
	if conf.Pprof.Enabled {
		preLog.Info("fury is starting with pprof profile, this is not a recommended setting for production")
	...
	if l.conf.SQLStore.UseEmbedded {
		...
		runtimeDir := l.furyPaths.StatePathFor(paths.DataNodeEmbeddedPostgresRuntimeDir)
	...
	if l.conf.SQLStore.WipeOnStartup {
		if ResetDatabaseAndNetworkHistory(l.ctx, l.Log, l.furyPaths, l.conf.SQLStore.ConnectionConfig); err != nil {
...
func (l *NodeCommand) initialiseDatabase(preLog *logging.Logger) error {
	...
	hasFurySchema, err := sqlstore.HasFurySchema(l.ctx, pool)
	if err != nil {
	...
	if !hasFurySchema {
		err = sqlstore.RecreateFuryDatabase(l.ctx, preLog, l.conf.SQLStore.ConnectionConfig)
		if err != nil {
			return fmt.Errorf("failed to recreate fury schema: %w", err)
		}
	}
...
func (l *NodeCommand) preRun([]string) (err error) {
	...
	if l.conf.Broker.UseBufferedEventSource {
		bufferFilePath, err := l.furyPaths.CreateStatePathFor(paths.DataNodeEventBufferHome)
		if err != nil {
			preLog.Error("failed to create path for buffered event source", logging.Error(err))
			return err
		}

		archiveFilesPath, err := l.furyPaths.CreateStatePathFor(paths.DataNodeArchivedEventBufferHome)
		if err != nil {
	...
	l.furyCoreServiceClient = furyprotoapi.NewCoreServiceClient(conn)
	return nil
}
...
func (l *NodeCommand) initialiseNetworkHistory(preLog *logging.Logger, connConfig sqlstore.ConnectionConfig) error {
	...
	l.snapshotService, err = snapshot.NewSnapshotService(snapshotServiceLog, l.conf.NetworkHistory.Snapshot,
		networkHistoryPool,
		l.furyPaths.StatePathFor(paths.DataNodeNetworkHistorySnapshotCopyTo), func(version int64) error {
	...
	l.networkHistoryService, err = networkhistory.New(l.ctx, networkHistoryServiceLog, l.conf.NetworkHistory,
		l.furyPaths.StatePathFor(paths.DataNodeNetworkHistoryHome),
		networkHistoryPool,
		l.conf.ChainID, l.snapshotService, l.conf.API.Port, l.furyPaths.StatePathFor(paths.DataNodeNetworkHistorySnapshotCopyFrom),
		l.furyPaths.StatePathFor(paths.DataNodeNetworkHistorySnapshotCopyTo), l.conf.MaxMemoryPercent)
	if err != nil {
...
func ResetDatabaseAndNetworkHistory(ctx context.Context, log *logging.Logger, furyPaths paths.Paths, connConfig sqlstore.ConnectionConfig) error {
	err := os.RemoveAll(furyPaths.StatePathFor(paths.DataNodeNetworkHistoryHome))
	if err != nil {
	...
	if err := sqlstore.RecreateFuryDatabase(ctx, log, connConfig); err != nil {
