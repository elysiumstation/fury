...
import (
	...
	"github.com/elysiumstation/fury/libs/memory"

	"github.com/elysiumstation/fury/cmd/fury/commands/node"
...
func (cmd *StartCmd) Execute([]string) error {
	...
	confWatcher, err := config.NewWatcher(context.Background(), logCore, furyPaths, config.Use(parseFlagOpt))
	if err != nil {
		return err
	}
...
func Start(ctx context.Context, parser *flags.Parser) error {
	...
	cmd, err := parser.AddCommand("start", "Start a fury instance", "Runs a fury node", &startCmd)
	if err != nil {
		return err
	}
...
func Node(ctx context.Context, parser *flags.Parser) error {
	...
	cmd, err := parser.AddCommand("node", "deprecated, see fury start instead", "deprecated, use fury start instead", &startCmd)
	if err != nil {
		return err
	}
