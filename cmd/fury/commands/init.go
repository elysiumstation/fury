...
import (
	...
	"github.com/elysiumstation/fury/core/config/encoding"
...
type InitCmd struct {
	...
	Force bool `short:"f" long:"force" description:"Erase existing fury configuration at the specified path"`
...
func (opts *InitCmd) Execute(args []string) error {
	...
	if mode == encoding.NodeModeValidator {
		...
		nwRegistry, err = registry.NewLoader(furyPaths, pass)
		if err != nil {
			return err
		}
	}

	cfgLoader, err := config.InitialiseLoader(furyPaths)
	if err != nil {
...
func Init(ctx context.Context, parser *flags.Parser) error {
	...
	var (
		short = "Initializes a fury node"
		long  = "Generate the minimal configuration required for a fury node to start. You must specify 'full' or 'validator'"
	)
