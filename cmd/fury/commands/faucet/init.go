...
import (
	...
	"github.com/elysiumstation/fury/core/faucet"
...
type faucetInit struct {
	...
	UpdateInPlace bool `long:"update-in-place" description:"Update the Fury node configuration with the faucet public key"`
}
...
func (opts *faucetInit) Execute(_ []string) error {
	...
	initResult, err := faucet.Initialise(furyPaths, pass, opts.Force)
	if err != nil {
	...
	if opts.UpdateInPlace {
		nodeCfgLoader, nodeCfg, err := config.EnsureNodeConfig(furyPaths)
		if err != nil {
			return err
		}
