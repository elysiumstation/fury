...
func verifyValidators(r *reporter, validators map[string]validator) {
	...
	for tmkey, v := range validators {
		...
		if !isValidParty(v.FuryPubKey) {
			r.Err("app_state.validators[%v] has an invalid fury public key, `%v`", tmkey, v.FuryPubKey)
		}

		if v.FuryPubKeyIndex == 0 {
			r.Err("app_state.validators[%v] has an invalid fury public key index, `%v`", tmkey, v.FuryPubKeyIndex)
		}
