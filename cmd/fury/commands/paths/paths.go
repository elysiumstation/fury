...
type Cmd struct {
	List    ListCmd    `command:"list" description:"List the location where files used by the Fury applications are stored"`
...
func Paths(ctx context.Context, parser *flags.Parser) error {
	...
	var (
		short = "Manages the Fury paths"
		long  = `
			Fury applications store their configuration and their data at specific locations.
			...
			Fury applications also support setting a custom location for these files using
			the --home flag. Contrary to the XDG Base Directory specification, this flag
			will group the cache, config, data and state folders under a "fury" folder, at
