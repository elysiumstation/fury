...
func (opts *AnnounceNodeCmd) Execute(_ []string) error {
	...
	if err := validators.SignAnnounceNode(
		&cmd, nodeWallets.Fury, nodeWallets.Ethereum,
	); err != nil {
		return err
	}
	...
	if output.IsHuman() {
		...
		return vgjson.Print(struct {
			...
			FurySignature     string `json:"furySignature"`
		}{
			TxHash:            txHash,
			EthereumSignature: cmd.EthereumSignature.Value,
			FurySignature:     cmd.FurySignature.Value,
		})
	}
...
func getNodeWalletCommander(log *logging.Logger, registryPass string, furyPaths paths.Paths) (*nodewallets.Commander, *api.LastBlockHeightResponse, context.CancelFunc, error) {
	_, cfg, err := config.EnsureNodeConfig(furyPaths)
	if err != nil {
		return nil, nil, nil, err
	}

	furyWallet, err := nodewallets.GetFuryWallet(furyPaths, registryPass)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("couldn't get Fury node wallet: %w", err)
	}
	...
	commander, err := nodewallets.NewCommander(cfg.NodeWallet, log, blockchain.NewClientWithImpl(abciClient), furyWallet, heightProvider{height: resp.Height})
	if err != nil {
