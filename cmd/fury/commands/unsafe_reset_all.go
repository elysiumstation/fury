...
func (cmd *UnsafeResetAllCmd) Execute(_ []string) error {
	...
	snapshotsPath := furyPaths.StatePathFor(paths.SnapshotStateHome)
	...
	checkpointsPath := furyPaths.StatePathFor(paths.CheckpointStateHome)
...
func UnsafeResetAll(ctx context.Context, parser *flags.Parser) error {
	...
	_, err := parser.AddCommand("unsafe_reset_all", "(unsafe) Remove all application state", "(unsafe) Remove all fury application state (checkpoints and snapshots)", &unsafeResetCmd)
	return err
}
