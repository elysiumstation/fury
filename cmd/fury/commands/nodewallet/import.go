...
type importCmd struct {
	...
	Chain      string              `short:"c" long:"chain" required:"true" description:"The chain to be imported" choice:"fury" choice:"ethereum" choice:"tendermint"`
	...
	// clef flags
	EthereumClefAddress string `long:"ethereum-clef-address" description:"The URL to the clef instance that Fury will use."`
	EthereumClefAccount string `long:"ethereum-clef-account" description:"The Ethereum account to be imported by Fury from Clef. In hex."`
...
func (opts *importCmd) Execute(_ []string) error {
	...
	if opts.Chain == furyChain || (opts.Chain == ethereumChain && opts.EthereumClefAddress == "") {
	...
	case furyChain:
		data, err = nodewallets.ImportFuryWallet(furyPaths, registryPass, walletPass, walletPath, opts.Force)
		if err != nil {
			return fmt.Errorf("couldn't import Fury node wallet: %w", err)
		}
	case tendermintChain:
		...
		data, err = nodewallets.ImportTendermintPubkey(furyPaths, registryPass, tendermintPubkey, opts.Force)
		if err != nil {
