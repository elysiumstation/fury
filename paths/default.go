...
// The default Fury file structure is mapped on the XDG standard. This standard
...
//
// At each location, Fury files are grouped under a `fury` folder, as follows
// `$XDG_*/fury`, before being sorted in sub-folders. The file structure of
...
func CreateDefaultCachePathFor(relFilePath CachePath) (string, error) {
	path, err := xdg.CacheFile(filepath.Join(FuryHome, relFilePath.String()))
	if err != nil {
...
func CreateDefaultCacheDirFor(relDirPath CachePath) (string, error) {
	...
	path, err := xdg.CacheFile(filepath.Join(FuryHome, relDirPath.String(), "fake-file"))
	if err != nil {
...
func CreateDefaultConfigPathFor(relFilePath ConfigPath) (string, error) {
	path, err := xdg.ConfigFile(filepath.Join(FuryHome, relFilePath.String()))
	if err != nil {
		return "", fmt.Errorf("couldn't create the default directory for file: %w", err)
	}
	return path, nil
...
func CreateDefaultConfigDirFor(relDirPath ConfigPath) (string, error) {
	// We append fake-file to xdg library creates all directory up to fake-file.
	path, err := xdg.ConfigFile(filepath.Join(FuryHome, relDirPath.String(), "fake-file"))
	if err != nil {
		return "", fmt.Errorf("couldn't create the default directory: %w", err)
	}
	return filepath.Dir(path), nil
...
func CreateDefaultDataPathFor(relFilePath DataPath) (string, error) {
	path, err := xdg.DataFile(filepath.Join(FuryHome, relFilePath.String()))
	if err != nil {
		return "", fmt.Errorf("couldn't create the default directory for file: %w", err)
	}
	return path, nil
...
func CreateDefaultDataDirFor(relDirPath DataPath) (string, error) {
	// We append fake-file to xdg library creates all directory up to fake-file.
	path, err := xdg.DataFile(filepath.Join(FuryHome, relDirPath.String(), "fake-file"))
	if err != nil {
		return "", fmt.Errorf("couldn't create the default directory: %w", err)
	}
	return filepath.Dir(path), nil
...
func CreateDefaultStatePathFor(relFilePath StatePath) (string, error) {
	path, err := xdg.StateFile(filepath.Join(FuryHome, relFilePath.String()))
	if err != nil {
		return "", fmt.Errorf("couldn't create the default directory for file: %w", err)
	}
	return path, nil
...
func CreateDefaultStateDirFor(relDirPath StatePath) (string, error) {
	// We append fake-file to xdg library creates all directory up to fake-file.
	path, err := xdg.StateFile(filepath.Join(FuryHome, relDirPath.String(), "fake-file"))
	if err != nil {
		return "", fmt.Errorf("couldn't create the default directory: %w", err)
	}
	return filepath.Dir(path), nil
...
func DefaultCachePathFor(relPath CachePath) string {
	return filepath.Join(xdg.CacheHome, FuryHome, relPath.String())
}
...
func DefaultStatePathFor(relPath StatePath) string {
	return filepath.Join(xdg.StateHome, FuryHome, relPath.String())
}
