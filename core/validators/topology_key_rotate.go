...
var (
	...
	ErrNewFuryPubKeyIndexMustBeGreaterThenCurrentPubKeyIndex = errors.New("a new fury public key index must be greather then current public key index")
	ErrInvalidFuryPubKeyForNode                              = errors.New("current fury public key is invalid for node")
...
func (t *Topology) AddKeyRotate(ctx context.Context, nodeID string, currentBlockHeight uint64, kr *commandspb.KeyRotateSubmission) error {
	...
	if node.data.FuryPubKeyIndex >= kr.NewPubKeyIndex {
		return ErrNewFuryPubKeyIndexMustBeGreaterThenCurrentPubKeyIndex
	}

	hashedFuryPubKey, err := node.data.HashFuryPubKey()
	if err != nil {
		return err
	}

	if hashedFuryPubKey != kr.CurrentPubKeyHash {
...
func (t *Topology) keyRotationBeginBlockLocked(ctx context.Context) {
	...
	for _, nodeID := range nodeIDs {
		...
		if !ok {
			...
			t.log.Error("failed to rotate Fury key due to non present validator", logging.String("nodeID", nodeID))
			continue
		}

		oldPubKey := data.data.FuryPubKey
		rotation := t.pendingPubKeyRotations[t.currentBlockHeight][nodeID]

		data.data.FuryPubKey = rotation.newPubKey
		data.data.FuryPubKeyIndex = rotation.newKeyIndex
		t.validators[nodeID] = data

		t.notifyKeyChange(ctx, oldPubKey, rotation.newPubKey)
		t.broker.Send(events.NewFuryKeyRotationEvent(ctx, nodeID, oldPubKey, rotation.newPubKey, t.currentBlockHeight))
