...
func testRotateEthereumKeySuccess(t *testing.T) {
	...
	err = top.ProcessEthereumKeyRotation(ctx, nr.FuryPubKey, ekr, MockVerify)
	require.NoError(t, err)
}
...
func testRotateEthereumKeyFailsOnNonExistingNode(t *testing.T) {
	...
	err := top.ProcessEthereumKeyRotation(
		context.Background(),
		"fury-nonexisting-pubkey",
		newEthereumKeyRotationSubmission("", "new-eth-addr", 10, ""),
		MockVerify,
	)

	assert.Error(t, err)
	assert.EqualError(t, err, "failed to rotate ethereum key for non existing validator \"fury-nonexisting-pubkey\"")
}
...
func testEthereumKeyRotationBeginBlock(t *testing.T) {
	...
	err := top.ProcessEthereumKeyRotation(ctx, "fury-key-1", newEthereumKeyRotationSubmission("eth-address-1", "new-eth-address-1", 11, ""), MockVerify)
	require.NoError(t, err)
	err = top.ProcessEthereumKeyRotation(ctx, "fury-key-2", newEthereumKeyRotationSubmission("eth-address-2", "new-eth-address-2", 11, ""), MockVerify)
	require.NoError(t, err)
	err = top.ProcessEthereumKeyRotation(ctx, "fury-key-3", newEthereumKeyRotationSubmission("eth-address-3", "new-eth-address-3", 13, ""), MockVerify)
	require.NoError(t, err)
	err = top.ProcessEthereumKeyRotation(ctx, "fury-key-4", newEthereumKeyRotationSubmission("eth-address-4", "new-eth-address-4", 13, ""), MockVerify)
	require.NoError(t, err)
...
func TestEthereumKeyRotationBeginBlockWithSubmitter(t *testing.T) {
	...
	for i := 0; i < len(chainValidators); i++ {
		...
		nr := commandspb.AnnounceNode{
			Id:              id,
			ChainPubKey:     chainValidators[i],
			FuryPubKey:      fmt.Sprintf("fury-key-%d", j),
			EthereumAddress: fmt.Sprintf("eth-address-%d", j),
		}
	...
	// add ethereum key rotations
	err := top.ProcessEthereumKeyRotation(ctx, "fury-key-1", newEthereumKeyRotationSubmission("eth-address-1", "new-eth-address-1", 11, submitter), MockVerify)
	require.NoError(t, err)
	...
	err = top.ProcessEthereumKeyRotation(ctx, "fury-key-1", newEthereumKeyRotationSubmission("eth-address-1", "new-eth-address-1", 13, submitter), MockVerify)
	...
	// try to submit again
	err = top.ProcessEthereumKeyRotation(ctx, "fury-key-1", newEthereumKeyRotationSubmission("new-eth-address-1", "new-eth-address-2", 150, submitter), MockVerify)
	require.NoError(t, err)
}
