...
func addTwoNodes(top *testTop) {
	...
	for i := 0; i < number; i++ {
		top.AddNewNode(ctx, &commandspb.AnnounceNode{
			Id:              fmt.Sprintf("fury-master-pubkey-%d", i),
			ChainPubKey:     tmPubKeys[0],
			FuryPubKey:      hexEncode(fmt.Sprintf("fury-key-%d", i)),
...
func testTopologyCheckpointSuccess(t *testing.T) {
	...
	kr1 := &commandspb.KeyRotateSubmission{
		NewPubKeyIndex:    1,
		TargetBlock:       10,
		NewPubKey:         "new-fury-key",
		CurrentPubKeyHash: hashKey("fury-key-0"),
	}
	err := top.AddKeyRotate(ctx, "fury-master-pubkey-0", 5, kr1)
	assert.NoError(t, err)

	kr2 := &commandspb.KeyRotateSubmission{
		NewPubKeyIndex:    1,
		TargetBlock:       11,
		NewPubKey:         "new-fury-key-1",
		CurrentPubKeyHash: hashKey("fury-key-1"),
	}
	err = top.AddKeyRotate(ctx, "fury-master-pubkey-1", 5, kr2)
	assert.NoError(t, err)

	ekr1 := newEthereumKeyRotationSubmission("eth-address-0", "new-eth-address-0", 10, "")
	err = top.ProcessEthereumKeyRotation(ctx, hexEncode("fury-key-0"), ekr1, MockVerify)
	assert.NoError(t, err)

	ekr2 := newEthereumKeyRotationSubmission("eth-address-1", "new-eth-address-1", 11, "")
	err = top.ProcessEthereumKeyRotation(ctx, hexEncode("fury-key-1"), ekr2, MockVerify)
	assert.NoError(t, err)
	...
	assert.Equal(t, top.GetPendingEthereumKeyRotation(ekr1.TargetBlock, "fury-master-pubkey-0"), newTop.GetPendingEthereumKeyRotation(ekr1.TargetBlock, "fury-master-pubkey-0"))
	assert.Equal(t, top.GetPendingEthereumKeyRotation(ekr2.TargetBlock, "fury-master-pubkey-1"), newTop.GetPendingEthereumKeyRotation(ekr2.TargetBlock, "fury-master-pubkey-1"))
}
