...
import (
	...
	"github.com/elysiumstation/fury/core/validators/mocks"
...
type NodeWallets struct {
	fury             validators.Wallet
	tendermintPubkey string
	ethereumAddress  string
	ethereum         validators.Signer
}

func (n *NodeWallets) GetFury() validators.Wallet {
	return n.fury
}
	...
	state := struct {
		...
		Validators: map[string]validators.ValidatorData{
			defaultTmPubKeyBase64: {
				...
				FuryPubKey:      top.wallet.PubKey().Hex(),
				TmPubKey:        "asdasd",
				EthereumAddress: "0x123456",
				InfoURL:         "n0.xyz.fury/node/123",
				Country:         "GB",
			},
		},
	}
...
func getTestTopologyWithSelfValidatorData(
	...
	ctrl := gomock.NewController(t)
	pubKey := crypto.NewPublicKey(self.FuryPubKey, []byte(self.FuryPubKey))
	id := crypto.NewPublicKey(self.ID, []byte(self.FuryPubKey))
...
func testValidatorsValidation(t *testing.T) {
	self := validators.ValidatorData{
		...
		FuryPubKey:      "6a8325087e5bdf57b60cf06c3764e3c6a32840079fdc432a437ce32cd99316b5",
	...
	otherValidators := []validators.ValidatorData{
		{
			...
			FuryPubKey:      "f4686749895bf51c6df4092ef6be4279c384a3c380c24ea7a2fd20afc602a35d",
		...
		},
		{
			...
			FuryPubKey:      "10b06fec6398d9e9d542d7b7d36933a1e6f0bb0631b0e532681c05123d4bd5aa",
...
func testAddNewNodeFailure(t *testing.T) {
	...
	// Add node with existing FuryPubKey
	nr = commandspb.AnnounceNode{
		Id:              "fury-master-pubkey",
...
func testExists(t *testing.T) {
	...
	assert.False(t, top.IsValidatorFuryPubKey("fury-key"))
	assert.False(t, top.IsValidatorNodeID("fury-master-pubkey"))
	...
	assert.True(t, top.IsValidatorFuryPubKey("fury-key"))
	assert.True(t, top.IsValidatorNodeID("fury-master-pubkey"))
}
...
func testGetByKey(t *testing.T) {
	...
	nr := commandspb.AnnounceNode{
		...
		FuryPubKey:      "fury-key",
		EthereumAddress: "eth-address",
		InfoUrl:         "n0.xyz.fury/node/url/random",
		Country:         "CZ",
	}
	ctx := context.Background()
	err := top.AddNewNode(ctx, &nr, validators.ValidatorStatusTendermint)
...
func testAddNewNodeSendsValidatorUpdateEventToBroker(t *testing.T) {
	...
	defer cleanupFn()
	_, err := nodewallets.GenerateFuryWallet(furyPaths, "pass", "pass", false)
	require.NoError(t, err)
	wallet, err := nodewallets.GetFuryWallet(furyPaths, "pass")
	require.NoError(t, err)
...
func testAddKeyRotateSuccess(t *testing.T) {
	...
	id := "fury-master-pubkey"
	furyPubKey := "fury-key"
	newFuryPubKey := fmt.Sprintf("new-%s", furyPubKey)

	nr := commandspb.AnnounceNode{
		Id:              id,
		ChainPubKey:     tmPubKey,
		FuryPubKey:      hex.EncodeToString([]byte(furyPubKey)),
		EthereumAddress: "eth-address",
	}
	ctx := context.TODO()
	err := top.AddNewNode(ctx, &nr, validators.ValidatorStatusTendermint)
	...
	kr := &commandspb.KeyRotateSubmission{
		NewPubKeyIndex:    1,
		TargetBlock:       15,
		NewPubKey:         newFuryPubKey,
		CurrentPubKeyHash: hashKey(furyPubKey),
	}
...
func testAddKeyRotateSuccessFailsOnNonExistingNode(t *testing.T) {
	...
	id := "fury-master-pubkey"
	newFuryPubKey := "new-ega-key"

	ctx := context.TODO()

	err := top.AddKeyRotate(ctx, id, 10, newKeyRotationSubmission("", newFuryPubKey, 1, 10))
	assert.Error(t, err)
	assert.EqualError(t, err, "failed to add key rotate for non existing node \"fury-master-pubkey\"")
}
...
func testAddKeyRotateSuccessFailsWhenTargetBlockHeightIsLessThenCurrentBlockHeight(t *testing.T) {
	...
	err = top.AddKeyRotate(ctx, id, 15, newKeyRotationSubmission(furyPubKey, newFuryPubKey, 1, 10))
...
func testAddKeyRotateSuccessFailsWhenNewKeyIndexIsLessThenCurrentKeyIndex(t *testing.T) {
	...
	// test less then
	err = top.AddKeyRotate(ctx, id, 10, newKeyRotationSubmission(furyPubKey, newFuryPubKey, 1, 15))
	assert.ErrorIs(t, err, validators.ErrNewFuryPubKeyIndexMustBeGreaterThenCurrentPubKeyIndex)
}
...
func testAddKeyRotateSuccessFailsWhenKeyRotationForNodeAlreadyExists(t *testing.T) {
	...
	// add first
	err = top.AddKeyRotate(ctx, id, 10, newKeyRotationSubmission(furyPubKey, newFuryPubKey, 2, 12))
	assert.NoError(t, err)

	// add second
	err = top.AddKeyRotate(ctx, id, 10, newKeyRotationSubmission(furyPubKey, newFuryPubKey, 2, 13))
...
func testAddKeyRotateSuccessFailsWhenCurrentPubKeyHashDoesNotMatch(t *testing.T) {
	...
	err = top.AddKeyRotate(ctx, id, 10, newKeyRotationSubmission("random-key", newFuryPubKey, 2, 12))
...
func newKeyRotationSubmission(currentPubKey, newFuryPubKey string, keyIndex uint32, targetBlock uint64) *commandspb.KeyRotateSubmission {
...
func testBeginBlockSuccess(t *testing.T) {
	...
	for i := 0; i < len(chainValidators); i++ {
		j := i + 1
		id := fmt.Sprintf("fury-master-pubkey-%d", j)
		nr := commandspb.AnnounceNode{
			Id:              id,
			ChainPubKey:     chainValidators[i],
			FuryPubKey:      hex.EncodeToString([]byte(fmt.Sprintf("fury-key-%d", j))),
	...
	// add key rotations
	err := top.AddKeyRotate(ctx, "fury-master-pubkey-1", 10, newKeyRotationSubmission("fury-key-1", "new-fury-key-1", 1, 11))
	assert.NoError(t, err)
	err = top.AddKeyRotate(ctx, "fury-master-pubkey-2", 10, newKeyRotationSubmission("fury-key-2", "new-fury-key-2", 1, 11))
	assert.NoError(t, err)
	err = top.AddKeyRotate(ctx, "fury-master-pubkey-3", 10, newKeyRotationSubmission("fury-key-3", "new-fury-key-3", 1, 13))
	assert.NoError(t, err)
	err = top.AddKeyRotate(ctx, "fury-master-pubkey-4", 10, newKeyRotationSubmission("fury-key-4", "new-fury-key-4", 1, 13))
	assert.NoError(t, err)

	// when
	top.BeginBlock(ctx, abcitypes.RequestBeginBlock{Header: types1.Header{Height: 11}})
	// then
	data1 := top.Get("fury-master-pubkey-1")
	assert.NotNil(t, data1)
	assert.Equal(t, "new-fury-key-1", data1.FuryPubKey)
	data2 := top.Get("fury-master-pubkey-2")
	assert.NotNil(t, data2)
	assert.Equal(t, "new-fury-key-2", data2.FuryPubKey)
	data3 := top.Get("fury-master-pubkey-3")
	assert.NotNil(t, data3)
	assert.Equal(t, hex.EncodeToString([]byte("fury-key-3")), data3.FuryPubKey)
	data4 := top.Get("fury-master-pubkey-4")
	assert.NotNil(t, data4)
	assert.Equal(t, hex.EncodeToString([]byte("fury-key-4")), data4.FuryPubKey)
	...
	data3 = top.Get("fury-master-pubkey-3")
	assert.NotNil(t, data3)
	assert.Equal(t, "new-fury-key-3", data3.FuryPubKey)
	data4 = top.Get("fury-master-pubkey-4")
	assert.NotNil(t, data4)
	assert.Equal(t, "new-fury-key-4", data4.FuryPubKey)
}
