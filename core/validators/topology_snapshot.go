		...
		nodes = append(nodes,
			&snapshot.ValidatorState{
				ValidatorUpdate: &eventspb.ValidatorUpdate{
					NodeId:           node.data.ID,
					FuryPubKey:       node.data.FuryPubKey,
					FuryPubKeyIndex:  node.data.FuryPubKeyIndex,
...
func (t *Topology) restore(ctx context.Context, topology *types.Topology, p *types.Payload) error {
	...
	for _, node := range topology.ValidatorData {
		...
		vs := &valState{
			data: ValidatorData{
				...
				FuryPubKey:       node.ValidatorUpdate.FuryPubKey,
				FuryPubKeyIndex:  node.ValidatorUpdate.FuryPubKeyIndex,
	...
	bh, err := furyctx.BlockHeightFromContext(ctx)
	if err != nil {
