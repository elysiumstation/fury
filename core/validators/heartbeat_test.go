...
func TestProcessValidatorHeartbeat(t *testing.T) {
	...
	cmd.FurySignature = &commandspb.Signature{
		Value: "haha",
	}
	...
	cmd.FurySignature.Value = "abcdef"

	topology.validators["node1"].data.FuryPubKey = "fooo"
	require.Error(t, topology.ProcessValidatorHeartbeat(context.Background(), cmd, nil, nil))
	...
	topology.validators["node1"].data.FuryPubKey = "eeee"
	rejectFury := func(message, signature, pubkey []byte) error { return errors.New("unverifiable fury signature") }
	rejectEth := func(message, signature []byte, hexAddress string) error {
		return errors.New("unverifiable eth signature")
	}
	acceptFury := func(message, signature, pubkey []byte) error { return nil }
	acceptEth := func(message, signature []byte, hexAddress string) error { return nil }

	// unverifiable fury signature
	require.Error(t, topology.ProcessValidatorHeartbeat(context.Background(), cmd, rejectFury, nil))
	...
	require.Error(t, topology.ProcessValidatorHeartbeat(context.Background(), cmd, acceptFury, nil))
	...
	require.Error(t, topology.ProcessValidatorHeartbeat(context.Background(), cmd, acceptFury, rejectEth))
	...
	require.NoError(t, topology.ProcessValidatorHeartbeat(context.Background(), cmd, acceptFury, acceptEth))
