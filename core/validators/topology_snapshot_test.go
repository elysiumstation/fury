...
func TestTopologySnapshot(t *testing.T) {
	...
	nr2 := commandspb.AnnounceNode{
		Id:              "fury-master-pubkey-2",
		ChainPubKey:     tmPubKeys[1],
		FuryPubKey:      hexEncode("fury-key-2"),
		EthereumAddress: "0x6d53C489bbda35B8096C8b4Cb362e2889F82E19B",
	}
	...
	kr1 := &commandspb.KeyRotateSubmission{
		...
		CurrentPubKeyHash: hashKey("fury-key"),
	}
	...
	kr2 := &commandspb.KeyRotateSubmission{
		NewPubKeyIndex:    1,
		TargetBlock:       11,
		NewPubKey:         "new-fury-key-2",
		CurrentPubKeyHash: hashKey("fury-key-2"),
	}
	...
	err = top.ProcessEthereumKeyRotation(ctx, hexEncode("fury-key"), ekr1, MockVerify)
	assert.NoError(t, err)
	...
	err = top.ProcessEthereumKeyRotation(ctx, hexEncode("fury-key-2"), ekr2, MockVerify)
	assert.NoError(t, err)
	...
	ctx = furyctx.WithBlockHeight(ctx, 100)
	...
	assert.ElementsMatch(t, top.AllFuryPubKeys(), snapTop.AllFuryPubKeys())
