...
import (
	...
	v1 "github.com/elysiumstation/fury/protos/fury/snapshot/v1"
...
var (
	ErrFuryNodeAlreadyRegisterForChain = errors.New("a fury node is already registered with the blockchain node")
...
type ValidatorData struct {
	ID               string `json:"id"`
	FuryPubKey       string `json:"fury_pub_key"`
	FuryPubKeyIndex  uint32 `json:"fury_pub_key_index"`
...
func (v ValidatorData) IsValid() bool {
	if len(v.ID) <= 0 || len(v.FuryPubKey) <= 0 ||
...
// HashFuryPubKey returns hash FuryPubKey encoded as hex string.
func (v ValidatorData) HashFuryPubKey() (string, error) {
	decoded, err := hex.DecodeString(v.FuryPubKey)
	if err != nil {
...
// ValidatorMapping maps a tendermint pubkey with a fury pubkey.
...
// AllFuryPubKeys returns all the validators fury public keys.
func (t *Topology) AllFuryPubKeys() []string {
	t.mu.RLock()
	defer t.mu.RUnlock()
	keys := make([]string, 0, len(t.validators))
	for _, data := range t.validators {
		keys = append(keys, data.data.FuryPubKey)
	}
	return keys
}
...
func (t *Topology) SelfFuryPubKey() string {
	if !t.isValidatorSetup {
		return ""
	}
	return t.wallets.GetFury().PubKey().Hex()
}
...
func (t *Topology) SelfNodeID() string {
	...
	return t.wallets.GetFury().ID().Hex()
}
...
// IsValidatorFuryPubKey returns true if the given key is a Fury validator public key.
func (t *Topology) IsValidatorFuryPubKey(pubkey string) (ok bool) {
	defer func() {
		if t.log.GetLevel() <= logging.DebugLevel {
			...
			t.log.Debug(s,
				logging.Strings("validators", t.AllFuryPubKeys()),
	...
	for _, data := range t.validators {
		if data.data.FuryPubKey == pubkey {
			return true
		}
	}
...
func (t *Topology) IsSelfTendermintValidator() bool {
	return t.IsTendermintValidator(t.SelfFuryPubKey())
}
...
func (t *Topology) GetVotingPower(pubkey string) int64 {
	...
	defer t.mu.RUnlock()
	for _, data := range t.validators {
		if data.data.FuryPubKey == pubkey && data.status == ValidatorStatusTendermint {
...
// IsValidatorFuryPubKey returns true if the given key is a Fury validator public key and the validators is of status Tendermint.
...
func (t *Topology) AddNewNode(ctx context.Context, nr *commandspb.AnnounceNode, status ValidatorStatus) error {
	...
	if _, ok := t.validators[nr.Id]; ok {
		return ErrFuryNodeAlreadyRegisterForChain
	}

	data := ValidatorData{
		ID:               nr.Id,
		FuryPubKey:       nr.FuryPubKey,
		FuryPubKeyIndex:  nr.FuryPubKeyIndex,
	...
	t.log.Info("new node registration successful",
		...
		logging.String("fury-key", nr.FuryPubKey),
...
func (t *Topology) LoadValidatorsOnGenesis(ctx context.Context, rawstate []byte) (err error) {
	...
	// tm is base64 encoded, fury is hex
	for tm, data := range state {
		...
		nr := &commandspb.AnnounceNode{
			Id:              data.ID,
			FuryPubKey:      data.FuryPubKey,
			FuryPubKeyIndex: data.FuryPubKeyIndex,
...
func (t *Topology) checkValidatorDataWithSelfWallets(data ValidatorData) {
	...
	hasError := t.wallets.GetFury().ID().Hex() != data.ID ||
		t.wallets.GetFury().PubKey().Hex() != data.FuryPubKey ||
	...
	if hasError {
		t.log.Panic("invalid node wallet configurations, the genesis validator mapping differ to the wallets imported by the nodewallet",
			...
			logging.String("genesis-fury-pubkey", data.FuryPubKey),
			logging.String("nodewallet-fury-pubkey", t.wallets.GetFury().PubKey().Hex()),
			logging.String("genesis-fury-id", data.ID),
			logging.String("nodewallet-fury-id", t.wallets.GetFury().ID().Hex()),
