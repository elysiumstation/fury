...
func (t *Topology) ProcessValidatorHeartbeat(ctx context.Context, vh *commandspb.ValidatorHeartbeat,
	verifyFurySig func(message, signature, pubkey []byte) error,
	...
	furys, err := hex.DecodeString(vh.GetFurySignature().Value)
	if err != nil {
		validator.heartbeatTracker.recordHeartbeatResult(false)
		return err
	}
	furyPubKey, err := hex.DecodeString(node.data.FuryPubKey)
	if err != nil {
		validator.heartbeatTracker.recordHeartbeatResult(false)
		return err
	}
	if err := verifyFurySig([]byte(hash), furys, furyPubKey); err != nil {
		validator.heartbeatTracker.recordHeartbeatResult(false)
		return err
	}
...
func (t *Topology) prepareHeartbeat(blockHash string) *commandspb.ValidatorHeartbeat {
	...
	furySignature, err := t.wallets.GetFury().Sign(blockHashBytes)
	if err != nil {
		t.log.Error("could not sign heartbeat with fury wallet",
