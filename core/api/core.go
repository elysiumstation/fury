...
import (
	...
	ptypes "github.com/elysiumstation/fury/protos/fury"
...
// GetFuryTime returns the latest blockchain header timestamp, in UnixNano format.
// Example: "1568025900111222333" corresponds to 2019-09-09T10:45:00.111222333Z.
func (s *coreService) GetFuryTime(ctx context.Context, _ *protoapi.GetFuryTimeRequest) (*protoapi.GetFuryTimeResponse, error) {
	defer metrics.StartAPIRequestAndTimeGRPC("GetFuryTime")()
	s.svcMu.RLock()
	defer s.svcMu.RUnlock()

	return &protoapi.GetFuryTimeResponse{
...
// Statistics provides various blockchain and Fury statistics, including:
// Blockchain height, backlog length, current time, orders and trades per block, tendermint version
// Fury counts for parties, markets, order actions (amend, cancel, submit), Fury version.
func (s *coreService) Statistics(ctx context.Context, _ *protoapi.StatisticsRequest) (*protoapi.StatisticsResponse, error) {
	...
	genesisTime := ""
	if gt != nil {
		genesisTime = furytime.Format(*gt)
	}
	...
	stats := &protoapi.Statistics{
		...
		CurrentTime:           furytime.Format(time.Now()),
		FuryTime:              furytime.Format(epochTime),
		Uptime:                furytime.Format(s.stats.GetUptime()),
		...
		EpochStartTime:        furytime.Format(s.stats.GetEpochStartTime()),
		EpochExpiryTime:       furytime.Format(s.stats.GetEpochExpireTime()),
		ChainId:               chainID,
	}
