...
import (
	...
	erc20contract "github.com/elysiumstation/fury/core/contracts/erc20"
	bridgecontract "github.com/elysiumstation/fury/core/contracts/erc20_bridge_logic_restricted"
	"github.com/elysiumstation/fury/core/metrics"
	"github.com/elysiumstation/fury/core/types"
	vgerrors "github.com/elysiumstation/fury/libs/errors"
...
//go:generate go run github.com/golang/mock/mockgen -destination mocks/eth_client_mock.go -package mocks github.com/elysiumstation/fury/core/bridges ETHClient
...
//go:generate go run github.com/golang/mock/mockgen -destination mocks/eth_confirmations_mock.go -package mocks github.com/elysiumstation/fury/core/bridges EthConfirmations
...
func (e *ERC20LogicView) FindAssetList(
	...
	resp := "ok"
	defer func() {
		metrics.EthCallInc("find_asset_list", al.FuryAssetID, resp)
	}()
	...
	assetID := strings.TrimPrefix(al.FuryAssetID, "0x")

	for iter.Next() {
		if hex.EncodeToString(iter.Event.FuryAssetId[:]) == assetID &&
...
func (e *ERC20LogicView) FindDeposit(
	...
	resp := "ok"
	defer func() {
		metrics.EthCallInc("find_deposit", d.FuryAssetID, resp)
	}()
	...
	for iter.Next() {
		if hex.EncodeToString(iter.Event.FuryPublicKey[:]) == targetPartyID &&
...
func (e *ERC20LogicView) FindWithdrawal(
	...
	resp := "ok"
	defer func() {
		metrics.EthCallInc("find_withdrawal", w.FuryAssetID, resp)
	}()
...
func (e *ERC20LogicView) FindAssetLimitsUpdated(
	...
	resp := "ok"
	defer func() {
		metrics.EthCallInc("find_asset_limits_updated", update.FuryAssetID, resp)
	}()
