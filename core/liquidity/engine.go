...
import (
	...
	"github.com/elysiumstation/fury/core/liquidity/supplied"
...
//go:generate go run github.com/golang/mock/mockgen -destination mocks/mocks.go -package mocks github.com/elysiumstation/fury/core/liquidity RiskModel,PriceMonitor,IDGen
...
// TimeService provide the time of the fury node using the tm time.
//
//go:generate go run github.com/golang/mock/mockgen -destination mocks/time_service_mock.go -package mocks github.com/elysiumstation/fury/core/liquidity TimeService
...
func (e *Engine) createOrUpdateForParty(
	...
	if repriceFailure {
		...
		for _, order := range partyOrders {
			if !order.IsLiquidityOrder() && order.Status == fury.Order_STATUS_ACTIVE {
...
func (e *Engine) GetCurrentLiquidityScores(bestBid, bestAsk num.Decimal, minLpPrice, maxLpPrice *num.Uint) (map[string]num.Decimal, num.Decimal) {
	...
	for _, p := range provs {
		if p.Status != fury.LiquidityProvision_STATUS_ACTIVE {
			continue
		}
...
func (e *Engine) getAllActiveOrders(party string) []*types.Order {
	...
	orders := make([]*types.Order, 0, len(partyOrders))
	for _, order := range partyOrders {
		if order.Status == fury.Order_STATUS_ACTIVE {
			orders = append(orders, order)
		}
	}
	return orders
