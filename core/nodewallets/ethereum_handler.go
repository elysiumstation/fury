...
func GetEthereumWallet(furyPaths paths.Paths, registryPassphrase string) (*eth.Wallet, error) {
	...
	return GetEthereumWalletWithRegistry(furyPaths, registry)
}

func GetEthereumWalletWithRegistry(furyPaths paths.Paths, reg *registry.Registry) (*eth.Wallet, error) {
	...
	case registry.EthereumKeyStoreWallet:
		walletLoader, err := keystore.InitialiseWalletLoader(furyPaths)
		if err != nil {
...
func GenerateEthereumWallet(
	...
	if clefAddress != "" {
		...
		keyStoreLoader, err := keystore.InitialiseWalletLoader(furyPaths)
		if err != nil {
			return nil, fmt.Errorf("couldn't initialise Ethereum key store node wallet loader: %w", err)
		}
...
func ImportEthereumWallet(
	...
	if clefAddress != "" {
		...
		ethWalletLoader, err := keystore.InitialiseWalletLoader(furyPaths)
		if err != nil {
