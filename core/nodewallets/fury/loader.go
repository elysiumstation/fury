...
import (
	...
	storev1 "github.com/elysiumstation/fury/wallet/wallet/store/v1"
...
func InitialiseWalletLoader(furyPaths paths.Paths) (*WalletLoader, error) {
	walletHome, err := furyPaths.CreateDataDirFor(paths.FuryNodeWalletsDataHome)
	if err != nil {
		return nil, fmt.Errorf("couldn't get the directory path for %s: %w", paths.FuryNodeWalletsDataHome, err)
	}
...
func (l *WalletLoader) Generate(passphrase string) (*Wallet, map[string]string, error) {
	...
	walletName := fmt.Sprintf("fury.%v", time.Now().UnixNano())
...
func (l *WalletLoader) Import(sourceFilePath string, passphrase string) (*Wallet, map[string]string, error) {
	...
	destWalletName := fmt.Sprintf("fury.%v", time.Now().UnixNano())
...
func newWallet(loader loader, store *storev1.FileStore, walletName, passphrase string) (*Wallet, error) {
	...
	if keyPairCount := len(keyPairs); keyPairCount == 0 {
		return nil, fmt.Errorf("fury wallet for node requires to have 1 key pair, none found")
	} else if keyPairCount != 1 {
		return nil, fmt.Errorf("fury wallet for node requires to have max 1 key pair, found %v", keyPairCount)
	}
