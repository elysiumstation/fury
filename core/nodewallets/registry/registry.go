...
type Registry struct {
	...
	Fury       *RegisteredFuryWallet       `json:"fury,omitempty"`
}
...
type RegisteredFuryWallet struct {
	Name       string `json:"name"`
	Passphrase string `json:"passphrase"`
}
...
func NewLoader(furyPaths paths.Paths, passphrase string) (*Loader, error) {
	registryFilePath, err := furyPaths.CreateConfigPathFor(paths.NodeWalletsConfigFile)
	if err != nil {
