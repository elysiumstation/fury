...
func protocolUpgradeProposalID(upgradeBlockHeight uint64, furyReleaseTag string) string {
	return fmt.Sprintf("%v@%v", furyReleaseTag, upgradeBlockHeight)
}
...
func (e *Engine) IsValidProposal(ctx context.Context, pk string, upgradeBlockHeight uint64, furyReleaseTag string) error {
	...
	newv, err := semver.Parse(TrimReleaseTag(furyReleaseTag))
	if err != nil {
		err = fmt.Errorf("invalid protocol version for upgrade received: version (%s), %w", furyReleaseTag, err)
...
func (e *Engine) UpgradeProposal(ctx context.Context, pk string, upgradeBlockHeight uint64, furyReleaseTag string) error {
	e.lock.RLock()
	...
	e.log.Debug("Adding protocol upgrade proposal",
		...
		logging.String("furyReleaseTag", furyReleaseTag),
		logging.String("currentVersion", e.currentVersion),
	)

	if err := e.IsValidProposal(ctx, pk, upgradeBlockHeight, furyReleaseTag); err != nil {
		return err
	}

	ID := protocolUpgradeProposalID(upgradeBlockHeight, furyReleaseTag)
	...
	if furyReleaseTag != e.currentVersion {
	...
	for _, activeID := range activeIDs {
		...
		if _, ok := activeProposal.accepted[pk]; ok {
			...
			e.log.Debug("Removed validator vote from previous proposal",
				...
				logging.String("furyReleaseTag", activeProposal.furyReleaseTag),
			)
		}
...
func (e *Engine) sendAndKeepEvent(ctx context.Context, ID string, activeProposal *protocolUpgradeProposal) {
	...
	evt := events.NewProtocolUpgradeProposalEvent(ctx, activeProposal.blockHeight, activeProposal.furyReleaseTag, activeProposal.approvers(), status)
	evtProto := evt.Proto()
...
func (e *Engine) BeginBlock(ctx context.Context, blockHeight uint64) {
	...
	for _, ID := range e.getProposalIDs() {
		...
		if e.isAccepted(pup) {
			...
			if blockHeight >= pup.blockHeight {
				...
				delete(e.events, ID)
				e.log.Info("protocol upgrade rejected", logging.String("fury-release-tag", pup.furyReleaseTag), logging.Uint64("upgrade-block-height", pup.blockHeight))
				e.broker.Send(events.NewProtocolUpgradeProposalEvent(ctx, pup.blockHeight, pup.furyReleaseTag, pup.approvers(), eventspb.ProtocolUpgradeProposalStatus_PROTOCOL_UPGRADE_PROPOSAL_STATUS_REJECTED))
			}
		}
	}
	e.lock.Lock()
	...
	if accepted != nil {
		e.upgradeStatus.AcceptedReleaseInfo = &types.ReleaseInfo{
			FuryReleaseTag:     accepted.furyReleaseTag,
...
func (e *Engine) Cleanup(ctx context.Context) {
	...
	for _, ID := range e.getProposalIDs() {
		...
		e.broker.Send(events.NewProtocolUpgradeProposalEvent(ctx, pup.blockHeight, pup.furyReleaseTag, pup.approvers(), status))
		delete(e.activeProposals, ID)
		delete(e.events, ID)
	}
}
...
func (e *Engine) SetCoreReadyForUpgrade() {
	...
	e.log.Info("marking fury core as ready to shut down")
...
func (e *Engine) SetReadyForUpgrade() {
	...
	e.log.Info("marking fury core and data node as ready to shut down")
