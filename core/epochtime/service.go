...
// Svc represents the Service managing epoch inside Fury.
type Svc struct {
	config Config
...
func (s *Svc) OnTick(ctx context.Context, t time.Time) {
	...
	if s.epoch.StartTime.IsZero() {
		...
		s.epoch.Action = fury.EpochAction_EPOCH_ACTION_START
	...
	if s.readyToStartNewEpoch {
		...
		s.epoch.EndTime = t
		s.epoch.Action = fury.EpochAction_EPOCH_ACTION_END
		s.notify(ctx, s.epoch)
