...
import (
	...
	bmok "github.com/elysiumstation/fury/core/broker/mocks"
	"github.com/elysiumstation/fury/core/events"
	"github.com/elysiumstation/fury/core/oracles"
	"github.com/elysiumstation/fury/core/oracles/mocks"
...
func spec(t *testing.T, currency string, op datapb.Condition_Operator, price string, keys ...string) specBundle {
	...
	testSpec := furypb.NewDataSourceSpec(
		furypb.NewDataSourceDefinition(
...
func (b *testBroker) expectNewOracleSpecSubscription(currentTime time.Time, spec *types.OracleSpec) {
	...
	b.EXPECT().Send(events.NewOracleSpecEvent(b.ctx, furypb.OracleSpec{ExternalDataSourceSpec: proto})).Times(1)
}
...
func (b *testBroker) expectMatchedOracleDataEvent(currentTime time.Time, data *furypb.OracleData, specIDs []string) {
