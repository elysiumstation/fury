...
var (
	...
	// ErrInvalidPropertyKey is returned if validation finds a reserved Fury property key.
...
// Currently FURY network utilises internal triggers in the oracle function path, even though
...
func NewOracleSpec(originalSpec types.ExternalDataSourceSpec) (*OracleSpec, error) {
	...
	for _, f := range filtersFromSpec {
		if isExtType {
			...
			if strings.HasPrefix(f.Key.Name, "elysiumstation.builtin") && f.Key.Type == datapb.PropertyKey_TYPE_TIMESTAMP {
				builtInKey = true
			}
			...
			// Currently FURY network uses only one type of internal data source - time triggered
			// that uses the property name "elysiumstation.builtin.timestamp"
			// https://github.com/elysiumstation/specs/blob/master/protocol/0048-DSRI-data_source_internal.md#13-fury-time-changed
			...
			typedFilters[f.Key.Name] = &filter{
				propertyName: "elysiumstation.builtin.timestamp",
