...
//
//go:generate go run github.com/golang/mock/mockgen -destination mocks/time_service_mock.go -package mocks github.com/elysiumstation/fury/core/oracles TimeService
...
func (e *Engine) sendNewOracleSpecSubscription(ctx context.Context, update updatedSubscription) {
	proto := &furypb.ExternalDataSourceSpec{
	...
	proto.Spec.Status = furypb.DataSourceSpec_STATUS_ACTIVE
	e.broker.Send(events.NewOracleSpecEvent(ctx, furypb.OracleSpec{ExternalDataSourceSpec: proto}))
}
...
func (e *Engine) sendOracleSpecDeactivation(ctx context.Context, update updatedSubscription) {
	...
	proto.Spec.CreatedAt = update.specActivatedAt.UnixNano()
	proto.Spec.Status = furypb.DataSourceSpec_STATUS_DEACTIVATED
	e.broker.Send(events.NewOracleSpecEvent(ctx, furypb.OracleSpec{ExternalDataSourceSpec: proto}))
}
...
func (e *Engine) sendMatchedOracleData(ctx context.Context, data OracleData, specIDs []OracleSpecID) {
	...
	e.broker.Send(events.NewOracleDataEvent(ctx, furypb.OracleData{ExternalData: dataProto.ExternalData}))
}
