...
type liquidityMonitoring struct {
	config map[string]*furypb.LiquidityMonitoringParameters
}
...
func (l *liquidityMonitoring) Add(name string, params *furypb.LiquidityMonitoringParameters) error {
...
func (l *liquidityMonitoring) Get(name string) (*furypb.LiquidityMonitoringParameters, error) {
	config, ok := l.config[name]
	if !ok {
		return config, fmt.Errorf("no liquidity monitoring \"%s\" registered", name)
	}
	// Copy to avoid modification between tests.
	copyConfig := &furypb.LiquidityMonitoringParameters{}
