...
import (
	...
	"github.com/elysiumstation/fury/core/integration/steps/market/defaults"
...
type riskModels struct {
	simple    map[string]*furypb.TradableInstrument_SimpleRiskModel
	logNormal map[string]*furypb.TradableInstrument_LogNormalRiskModel
}
...
func newRiskModels(unmarshaler *defaults.Unmarshaler) *riskModels {
	...
	for name, contentReader := range simpleRiskModelReaders {
		...
		if err := models.AddSimple(name, instrument.RiskModel.(*furypb.TradableInstrument_SimpleRiskModel)); err != nil {
	...
	for name, contentReader := range logNormalRiskModelReaders {
		...
		if err := models.AddLogNormal(name, instrument.RiskModel.(*furypb.TradableInstrument_LogNormalRiskModel)); err != nil {
			panic(fmt.Errorf("failed to add default simple risk model %s: %v", name, err))
		}
	}
...
func (r *riskModels) AddSimple(name string, model *furypb.TradableInstrument_SimpleRiskModel) error {
...
func (r *riskModels) AddLogNormal(name string, model *furypb.TradableInstrument_LogNormalRiskModel) error {
...
func (r riskModels) LoadModel(name string, instrument *furypb.TradableInstrument) error {
	simpleModel, okSimple := r.simple[name]
	if okSimple {
		// Copy to avoid modification between tests.
		copyConfig := &furypb.TradableInstrument_SimpleRiskModel{}
	...
	if okLogNormal {
		// Copy to avoid modification between tests.
		copyConfig := &furypb.TradableInstrument_LogNormalRiskModel{}
