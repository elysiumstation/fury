...
import (
	...
	"github.com/elysiumstation/fury/core/idgeneration"
	"github.com/elysiumstation/fury/core/processor/ratelimit"
	"github.com/elysiumstation/fury/core/txn"
	"github.com/elysiumstation/fury/core/types"
	"github.com/elysiumstation/fury/core/types/statevar"
...
type ProtocolUpgradeService interface {
	...
	UpgradeProposal(ctx context.Context, pk string, upgradeBlockHeight uint64, furyReleaseTag string) error
	TimeForUpgrade() bool
	...
	IsValidProposal(ctx context.Context, pk string, upgradeBlockHeight uint64, furyReleaseTag string) error
}
...
func (app *App) OnInitChain(req tmtypes.RequestInitChain) tmtypes.ResponseInitChain {
	...
	if err := app.ghandler.OnGenesis(ctx, req.Time, req.AppStateBytes); err != nil {
		app.cancel()
		app.log.Fatal("couldn't initialise fury with the genesis block", logging.Error(err))
	}
...
func (app *App) OnEndBlock(req tmtypes.RequestEndBlock) (ctx context.Context, resp tmtypes.ResponseEndBlock) {
	...
	app.log.Debug("ABCI service END block completed",
		...
		logging.String("current-datetime", furytime.Format(app.currentTimestamp)),
		logging.String("previous-datetime", furytime.Format(app.previousTimestamp)),
	)
...
func (app *App) handleCheckpoint(cpt *types.CheckpointState) error {
	...
	cpFilePath, err := app.furyPaths.CreateStatePathFor(paths.StatePath(filepath.Join(paths.CheckpointStateHome.String(), cpFileName)))
	if err != nil {
...
func (app *App) removeOldCheckpoints() error {
	cpDirPath, err := app.furyPaths.CreateStatePathFor(paths.StatePath(paths.CheckpointStateHome.String()))
	if err != nil {
...
func (app *App) limitPubkey(pk string) (limit bool, isValidator bool) {
	...
	if app.top.IsValidatorFuryPubKey(pk) {
		return false, true
	}
...
func (app *App) CheckProtocolUpgradeProposal(ctx context.Context, tx abci.Tx) error {
	...
	return app.protocolUpgradeService.IsValidProposal(ctx, tx.PubKeyHex(), pu.UpgradeBlockHeight, pu.FuryReleaseTag)
}

func (app *App) RequireValidatorPubKey(ctx context.Context, tx abci.Tx) error {
	if !app.top.IsValidatorFuryPubKey(tx.PubKeyHex()) {
...
func (app *App) DeliverProtocolUpgradeCommand(ctx context.Context, tx abci.Tx) error {
	...
	return app.protocolUpgradeService.UpgradeProposal(ctx, tx.PubKeyHex(), pu.UpgradeBlockHeight, pu.FuryReleaseTag)
}
...
func (app *App) DeliverValidatorHeartbeat(ctx context.Context, tx abci.Tx) error {
	...
	return app.top.ProcessValidatorHeartbeat(ctx, an, signatures.VerifyFurySignature, signatures.VerifyEthereumSignature)
}
...
func (app *App) DeliverCancelOrder(ctx context.Context, tx abci.Tx, deterministicID string) error {
	...
	// Submit the cancel new order request to the Fury trading core
	idgen := idgeneration.New(deterministicID)
...
func (app *App) DeliverAmendLiquidityProvision(ctx context.Context, tx abci.Tx, deterministicID string) error {
	...
	// Submit the amend liquidity provision request to the Fury trading core
