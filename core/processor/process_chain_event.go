...
func (app *App) processChainEvent(
	...
	if !app.top.IsValidatorFuryPubKey(pubkey) {
...
func (app *App) processChainEventBuiltinAsset(ctx context.Context, ce *types.ChainEventBuiltin, id string, nonce uint64) error {
	...
	case *types.BuiltinAssetEventDeposit:
		if err := app.checkFuryAssetID(act.Deposit, "BuiltinAsset.Deposit"); err != nil {
			return err
		}
		return app.banking.DepositBuiltinAsset(ctx, act.Deposit, id, nonce)
	case *types.BuiltinAssetEventWithdrawal:
		if err := app.checkFuryAssetID(act.Withdrawal, "BuiltinAsset.Withdrawal"); err != nil {
			return err
		}
...
func (app *App) processChainEventERC20(
	...
	case *types.ERC20EventAssetList:
		act.AssetList.FuryAssetID = strings.TrimPrefix(act.AssetList.FuryAssetID, "0x")
		if err := app.checkFuryAssetID(act.AssetList, "ERC20.AssetList"); err != nil {
			return err
		}
		// now check that the notary is GO for this asset
		_, ok := app.notary.IsSigned(
			ctx,
			act.AssetList.FuryAssetID,
	...
	case *types.ERC20EventDeposit:
		act.Deposit.FuryAssetID = strings.TrimPrefix(act.Deposit.FuryAssetID, "0x")
		if err := app.checkFuryAssetID(act.Deposit, "ERC20.AssetDeposit"); err != nil {
			return err
		}
		return app.banking.DepositERC20(ctx, act.Deposit, id, evt.Block, evt.Index, txID)
	case *types.ERC20EventWithdrawal:
		act.Withdrawal.FuryAssetID = strings.TrimPrefix(act.Withdrawal.FuryAssetID, "0x")
		if err := app.checkFuryAssetID(act.Withdrawal, "ERC20.AssetWithdrawal"); err != nil {
			return err
		}
	...
	case *types.ERC20EventAssetLimitsUpdated:
		act.AssetLimitsUpdated.FuryAssetID = strings.TrimPrefix(act.AssetLimitsUpdated.FuryAssetID, "0x")
		if err := app.checkFuryAssetID(act.AssetLimitsUpdated, "ERC20.AssetLimitsUpdated"); err != nil {
			return err
		}
...
type HasFuryAssetID interface {
	GetFuryAssetID() string
}

func (app *App) checkFuryAssetID(a HasFuryAssetID, action string) error {
	id := a.GetFuryAssetID()
	if _, err := app.assets.Get(id); err != nil {
		app.log.Error("invalid fury asset ID",
