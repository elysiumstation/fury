...
import (
	...
	"github.com/elysiumstation/fury/core/notary/mocks"
...
func testAddBadSignatureForOKResource(t *testing.T) {
	...
	notr.top.EXPECT().IsValidatorFuryPubKey(gomock.Any()).AnyTimes().Return(false)
...
func testAddKeyFinalize(t *testing.T) {
	...
	notr.top.EXPECT().IsValidatorFuryPubKey(gomock.Any()).AnyTimes().Return(true)
	...
	// first try to add a key for invalid resource
	notr.top.EXPECT().SelfFuryPubKey().Times(1).Return(key)
	err := notr.RegisterSignature(context.Background(), key, ns)
