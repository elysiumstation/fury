...
func TestNotarySnapshotRoundTrip(t *testing.T) {
	...
	notr.top.EXPECT().IsValidator().AnyTimes().Return(true)
	notr.top.EXPECT().SelfFuryPubKey().AnyTimes().Return("123456")
	...
	snapNotr.top.EXPECT().SelfFuryPubKey().AnyTimes().Return("123456")
	snapNotr.top.EXPECT().IsValidatorFuryPubKey(gomock.Any()).AnyTimes().Return(true)
