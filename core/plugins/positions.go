...
func (p *Positions) applyDistressedPositions(e DPE) {
	...
	for _, party := range e.DistressedParties() {
		if pos, ok := partyPos[party]; ok {
			pos.state = fury.PositionStatus_POSITION_STATUS_DISTRESSED
			partyPos[party] = pos
		}
	}
	for _, party := range e.SafeParties() {
		if pos, ok := partyPos[party]; ok {
			pos.state = fury.PositionStatus_POSITION_STATUS_UNSPECIFIED
			partyPos[party] = pos
		}
	}
...
func (p *Positions) applyDistressedOrders(e DOC) {
	...
	for _, party := range parties {
		if pos, ok := partyPos[party]; ok {
			pos.state = fury.PositionStatus_POSITION_STATUS_ORDERS_CLOSED
			partyPos[party] = pos
		}
	}
	p.data[marketID] = partyPos
...
func (p *Positions) updateSettleDestressed(e SDE) {
	...
	calc.Position.UpdatedAt = e.Timestamp()
	calc.state = fury.PositionStatus_POSITION_STATUS_CLOSED_OUT
	p.data[mID][tID] = calc
}
...
func (p *Positions) GetStateByMarketAndParty(market, party string) (fury.PositionStatus, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	mp, ok := p.data[market]
	if !ok {
		return fury.PositionStatus_POSITION_STATUS_UNSPECIFIED, nil
	}
...
func (p *Positions) GetPositionStatesByParty(party string) ([]fury.PositionStatus, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	// max 1 state per market
	states := make([]fury.PositionStatus, 0, len(p.data))
	for _, parties := range p.data {
		if pos, ok := parties[party]; ok {
