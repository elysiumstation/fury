...
func isSimilar(dispatchStrategy1, dispatchStrategy2 *furypb.DispatchStrategy) bool {
...
func (e *Engine) getMarketScores(ds *furypb.DispatchStrategy, payoutAsset, funder string) []*types.MarketContributionScore {
	switch ds.Metric {
	case furypb.DispatchMetric_DISPATCH_METRIC_MAKER_FEES_PAID, furypb.DispatchMetric_DISPATCH_METRIC_MAKER_FEES_RECEIVED, furypb.DispatchMetric_DISPATCH_METRIC_LP_FEES_RECEIVED:
		return e.marketActivityTracker.GetMarketScores(ds.AssetForMetric, ds.Markets, ds.Metric)

	case furypb.DispatchMetric_DISPATCH_METRIC_MARKET_VALUE:
		...
		if v.DispatchStrategy == nil {
			...
			if _, err = e.ensureFeeForTransferFunds(amount, v.From, v.Asset, v.FromAccountType); err == nil {
				...
				for _, fms := range marketScores {
					...
					if v.DispatchStrategy.Metric == furypb.DispatchMetric_DISPATCH_METRIC_MARKET_VALUE && fms.Score.IsPositive() {
