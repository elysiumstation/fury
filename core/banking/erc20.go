...
func (e *Engine) EnableERC20(
	...
	asset, _ := e.assets.Get(al.FuryAssetID)
	if _, ok := e.assetActs[al.FuryAssetID]; ok {
		e.log.Error("asset already being listed", logging.AssetID(al.FuryAssetID))
...
func (e *Engine) UpdateERC20(
	...
	asset, err := e.assets.Get(event.FuryAssetID)
	if err != nil {
		e.log.Panic("couldn't retrieve the ERC20 asset",
			logging.AssetID(event.FuryAssetID),
		)
	}
	aa := &assetAction{
		id:                      id,
