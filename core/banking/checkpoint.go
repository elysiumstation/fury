...
func (e *Engine) loadAssetActions(aa []*types.AssetAction) {
	for _, v := range aa {
		...
		if v.BuiltinD != nil {
			e.deposits[v.ID] = e.newDeposit(v.ID, v.BuiltinD.PartyID, v.BuiltinD.FuryAssetID, v.BuiltinD.Amount, v.Hash)
		} else if v.Erc20D != nil {
			e.deposits[v.ID] = e.newDeposit(v.ID, v.Erc20D.TargetPartyID, v.Erc20D.FuryAssetID, v.Erc20D.Amount, v.Hash)
		}
	}
}
