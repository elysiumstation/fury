...
//go:generate go run github.com/golang/mock/mockgen -destination mocks/mocks.go -package mocks github.com/elysiumstation/fury/core/banking Assets,Notary,Collateral,Witness,TimeService,EpochService,Topology,MarketActivityTracker,ERC20BridgeView,EthereumEventSource
...
func (e *Engine) finalizeAction(ctx context.Context, aa *assetAction) error {
	...
	case aa.IsERC20AssetList():
		return e.finalizeAssetList(ctx, aa.erc20AL.FuryAssetID)
	case aa.IsERC20AssetLimitsUpdated():
		return e.finalizeAssetLimitsUpdated(ctx, aa.erc20AssetLimitsUpdated.FuryAssetID)
