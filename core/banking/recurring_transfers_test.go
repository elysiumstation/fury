...
import (
	...
	"github.com/elysiumstation/fury/core/banking"
...
func TestMaturation(t *testing.T) {
	...
	e.OnEpoch(context.Background(), types.Epoch{Seq: 10, Action: fury.EpochAction_EPOCH_ACTION_START})
	e.OnEpoch(context.Background(), types.Epoch{Seq: 10, Action: fury.EpochAction_EPOCH_ACTION_END})
	e.OnEpoch(context.Background(), types.Epoch{Seq: 11, Action: fury.EpochAction_EPOCH_ACTION_START})
	e.OnEpoch(context.Background(), types.Epoch{Seq: 11, Action: fury.EpochAction_EPOCH_ACTION_END})
	e.OnEpoch(context.Background(), types.Epoch{Seq: 12, Action: fury.EpochAction_EPOCH_ACTION_START})
	e.OnEpoch(context.Background(), types.Epoch{Seq: 12, Action: fury.EpochAction_EPOCH_ACTION_END})
...
func testInvalidRecurringTransfersInThePast(t *testing.T) {
	...
	e.OnEpoch(context.Background(), types.Epoch{Seq: 7, Action: fury.EpochAction_EPOCH_ACTION_START})
...
func testInvalidRecurringTransfersDuplicates(t *testing.T) {
	...
	e.OnTransferFeeFactorUpdate(context.Background(), num.NewDecimalFromFloat(0.5))
	e.OnEpoch(context.Background(), types.Epoch{Seq: 7, Action: fury.EpochAction_EPOCH_ACTION_START})
	e.OnEpoch(context.Background(), types.Epoch{Seq: 7, Action: fury.EpochAction_EPOCH_ACTION_END})
...
func testForeverTransferCancelledNotEnoughFunds(t *testing.T) {
	...
	transfer := &types.TransferFunds{
		...
		Recurring: &types.RecurringTransfer{
			...
			DispatchStrategy: &fury.DispatchStrategy{Metric: fury.DispatchMetric_DISPATCH_METRIC_MAKER_FEES_RECEIVED},
			StartEpoch:       10,
	...
	e.marketActivityTracker.EXPECT().GetMarketScores(gomock.Any(), gomock.Any(), gomock.Any()).AnyTimes().Return([]*types.MarketContributionScore{
		{Asset: "asset1", Market: "market1", Metric: fury.DispatchMetric_DISPATCH_METRIC_MAKER_FEES_RECEIVED, Score: num.DecimalFromFloat(1)},
	})
	e.assets.EXPECT().Get(gomock.Any()).AnyTimes().Return(assets.NewAsset(&mockAsset{quantum: num.DecimalFromFloat(10)}), nil)
	e.tsvc.EXPECT().GetTimeNow().Times(2)
	e.broker.EXPECT().Send(gomock.Any()).Times(1)
	...
	e.OnEpoch(context.Background(), types.Epoch{Seq: 8, Action: fury.EpochAction_EPOCH_ACTION_START})
	e.OnEpoch(context.Background(), types.Epoch{Seq: 8, Action: fury.EpochAction_EPOCH_ACTION_END})
	e.OnEpoch(context.Background(), types.Epoch{Seq: 9, Action: fury.EpochAction_EPOCH_ACTION_START})
	e.OnEpoch(context.Background(), types.Epoch{Seq: 9, Action: fury.EpochAction_EPOCH_ACTION_END})
