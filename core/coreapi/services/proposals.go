...
func NewProposals(ctx context.Context) (proposals *Proposals) {
	...
	return &Proposals{
		...
		proposals:            map[string]furypb.Proposal{},
		proposalsPerProposer: map[string]map[string]struct{}{},
		ch:                   make(chan furypb.Proposal, 100),
	}
}
...
func (p *Proposals) List(proposal, party string) []*furypb.Proposal {
	p.mu.RLock()
	defer p.mu.RUnlock()
...
func (p *Proposals) getProposalsPerParty(proposal, party string) []*furypb.Proposal {
	out := []*furypb.Proposal{}
...
func (p *Proposals) getProposalByID(proposal string) []*furypb.Proposal {
	out := []*furypb.Proposal{}
...
func (p *Proposals) getAllProposals() []*furypb.Proposal {
	out := make([]*furypb.Proposal, 0, len(p.proposals))
