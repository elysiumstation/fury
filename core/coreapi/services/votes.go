...
type Votes struct {
	*subscribers.Base
	...
	votes map[string]map[string]furypb.Vote
...
func (v *Votes) List(proposal, party string) ([]*furypb.Vote, error) {
	v.mu.RLock()
	defer v.mu.RUnlock()
...
func (v *Votes) getVotesPerProposalAndParty(proposal, party string) []*furypb.Vote {
	out := []*furypb.Vote{}
...
func (v *Votes) getPartyVotes(party string) []*furypb.Vote {
	partyVotes, ok := v.votesPerParty[party]
	if !ok {
		return nil
	}

	out := make([]*furypb.Vote, 0, len(partyVotes))
...
func (v *Votes) getProposalVotes(proposal string) []*furypb.Vote {
	proposalVotes, ok := v.votes[proposal]
	if !ok {
		return nil
	}

	out := make([]*furypb.Vote, 0, len(proposalVotes))
