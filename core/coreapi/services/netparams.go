...
type netParamsE interface {
	events.Event
	NetworkParameter() furypb.NetworkParameter
}
...
type NetParams struct {
	*subscribers.Base
	...
	mu        sync.RWMutex
	netParams map[string]furypb.NetworkParameter
	ch        chan furypb.NetworkParameter
}
...
func NewNetParams(ctx context.Context) (netParams *NetParams) {
	...
	return &NetParams{
		...
		ctx:       ctx,
		netParams: map[string]furypb.NetworkParameter{},
		ch:        make(chan furypb.NetworkParameter, 100),
	}
}
...
func (a *NetParams) List(netParamsID string) []*furypb.NetworkParameter {
	a.mu.RLock()
	defer a.mu.RUnlock()
...
func (a *NetParams) getNetParam(param string) []*furypb.NetworkParameter {
	out := []*furypb.NetworkParameter{}
...
func (a *NetParams) getAllNetParams() []*furypb.NetworkParameter {
	out := make([]*furypb.NetworkParameter, 0, len(a.netParams))
