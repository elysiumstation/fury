...
type Assets struct {
	*subscribers.Base
	...
	mu     sync.RWMutex
	assets map[string]furypb.Asset
	ch     chan furypb.Asset
}
...
func (a *Assets) List(assetID string) []*furypb.Asset {
	a.mu.RLock()
	defer a.mu.RUnlock()
	if len(assetID) > 0 {
...
func (a *Assets) getAsset(assetID string) []*furypb.Asset {
	out := []*furypb.Asset{}
...
func (a *Assets) getAllAssets() []*furypb.Asset {
	out := make([]*furypb.Asset, 0, len(a.assets))
