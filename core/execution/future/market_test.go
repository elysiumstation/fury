...
import (
	...
	furycontext "github.com/elysiumstation/fury/libs/context"
	...
	"github.com/elysiumstation/fury/core/collateral"
	"github.com/elysiumstation/fury/core/events"
	"github.com/elysiumstation/fury/core/execution/common"
	"github.com/elysiumstation/fury/core/execution/common/mocks"
	"github.com/elysiumstation/fury/core/execution/future"
	"github.com/elysiumstation/fury/core/fee"
	"github.com/elysiumstation/fury/core/liquidity"
	"github.com/elysiumstation/fury/core/matching"
	"github.com/elysiumstation/fury/core/monitor"
	"github.com/elysiumstation/fury/core/oracles"
	"github.com/elysiumstation/fury/core/positions"
	"github.com/elysiumstation/fury/core/risk"
	"github.com/elysiumstation/fury/core/settlement"
	"github.com/elysiumstation/fury/core/types"
	"github.com/elysiumstation/fury/libs/num"
	"github.com/elysiumstation/fury/logging"
	proto "github.com/elysiumstation/fury/protos/fury"
...
func (tm *testMarket) WithSubmittedLiquidityProvision(t *testing.T, party string, amount uint64, fee string, buys, sells []*types.LiquidityOrder) *testMarket {
	t.Helper()
	ctx := furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash())
...
func TestMarketClosing(t *testing.T) {
	...
	tm.now = closingAt
	closed := tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), closingAt)
...
func TestMarketClosingAfterUpdate(t *testing.T) {
	...
	closed := tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), closingTimePlus1Sec)
	...
	closed = tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), closingTimePlus2Sec)
	...
	closed = tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), closingTimePlus3Sec)
...
func TestLiquidityFeeWhenTargetStakeDropsDueToFlowOfTime(t *testing.T) {
	...
	tm.now = now
	tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), now)
...
func TestTriggerByPriceNoTradesInAuction(t *testing.T) {
	...
	tm.now = openEnd
	tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), openEnd)
	now = openEnd
	...
	closed := tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), auctionEndTime)
	assert.False(t, closed)

	tm.now = afterAuction
	closed = tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), afterAuction)
...
func TestTriggerByPriceAuctionPriceOutsideBounds(t *testing.T) {
	...
	closed = tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), initialAuctionEnd)
	assert.False(t, closed)
...
func TestTriggerByMarketOrder(t *testing.T) {
	...
	// now leave auction
	tm.now = openingEnd
	tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), openingEnd)
	now = openingEnd
	...
	closed = tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), tm.now)
...
func TestTriggerAfterOpeningAuction(t *testing.T) {
	...
	closed := tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), afterOpeningAuction)
	assert.False(t, closed)
	auctionEnd = tm.market.GetMarketData().AuctionEnd
	require.Equal(t, int64(0), auctionEnd) // Not in auction
	...
	closed = tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), pMonitorAuctionEndTime)
	assert.False(t, closed)

	tm.now = afterPMonitorAuction
	closed = tm.market.OnTick(furycontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), afterPMonitorAuction)
	assert.False(t, closed)
}
...
func TestLPOrdersRollback(t *testing.T) {
	...
	t.Run("LiquidityProvision_REJECTED", func(t *testing.T) {
		...
		var found *furypb.LiquidityProvision
		for _, e := range tm.events {
			switch evt := e.(type) {
...
func Test3008And3007CancelLiquidityProvision(t *testing.T) {
	...
	t.Run("Fee are distribute to party-2 only", func(t *testing.T) {
		var found []*furypb.LedgerMovement
		for _, e := range tm.events {
			switch evt := e.(type) {
