...
func (mat *MarketActivityTracker) GetMarketScores(asset string, markets []string, dispatchMetric fury.DispatchMetric) []*types.MarketContributionScore {
...
func (mt *marketTracker) totalFees(metric fury.DispatchMetric) *num.Uint {
	switch metric {
	case fury.DispatchMetric_DISPATCH_METRIC_MAKER_FEES_RECEIVED:
		return mt.totalMakerFeesReceived
	case fury.DispatchMetric_DISPATCH_METRIC_MAKER_FEES_PAID:
		return mt.totalMakerFeesPaid
	case fury.DispatchMetric_DISPATCH_METRIC_LP_FEES_RECEIVED:
