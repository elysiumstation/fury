...
func (o *OnChainVerifier) CheckStakeDeposited(
	...
	decodedPubKeySlice, err := hex.DecodeString(event.FuryPubKey)
	if err != nil {
	...
	for _, address := range o.stakingBridgeAddresses {
		...
		furyPubKey := strings.TrimPrefix(event.FuryPubKey, "0x")
		...
		for iter.Next() {
			...
			if hex.EncodeToString(iter.Event.FuryPublicKey[:]) == furyPubKey &&
