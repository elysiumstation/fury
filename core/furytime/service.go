...
//go:generate go run github.com/golang/mock/mockgen -destination mocks/time_service_mock.go -package mocks github.com/elysiumstation/fury/core/furytime TimeService
...
// Svc represents the Service managing time inside Fury.
...
// New instantiates a new furytime service.
func New(conf Config, broker Broker) *Svc {
	return &Svc{config: conf, broker: broker}
}

// ReloadConf reload the configuration for the furytime service.
...
// GetTimeNow returns the current time in fury.
...
// which will be called once the fury time is updated (SetTimeNow is called).
...
// GetTimeLastBatch returns the previous fury time.
