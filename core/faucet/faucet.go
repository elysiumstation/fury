...
func Initialise(furyPaths paths.Paths, passphrase string, rewrite bool) (*InitialisationResult, error) {
	walletGenResult, err := GenerateWallet(furyPaths, passphrase)
	if err != nil {
		return nil, fmt.Errorf("couldn't generate a new faucet wallet: %w", err)
	}

	cfgLoader, err := InitialiseConfigLoader(furyPaths)
	if err != nil {
