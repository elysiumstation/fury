...
import (
	...
	bridge "github.com/elysiumstation/fury/core/contracts/erc20_bridge_logic_restricted"
	multisig "github.com/elysiumstation/fury/core/contracts/multisig_control"
...
// Assets ...
//
//go:generate go run github.com/golang/mock/mockgen -destination mocks/assets_mock.go -package mocks github.com/elysiumstation/fury/core/evtforward/ethereum Assets
type Assets interface {
	GetFuryIDFromEthereumAddress(string) string
}
...
// LogFilterer wraps the Ethereum event filterers to return Fury events.
			...
			Erc20: &vgproto.ERC20Event{
				...
				Action: &vgproto.ERC20Event_Withdrawal{
					Withdrawal: &vgproto.ERC20Withdrawal{
						FuryAssetId:           f.assets.GetFuryIDFromEthereumAddress(event.AssetSource.Hex()),
...
func (f *LogFilterer) debugAssetDeposited(event *bridge.Erc20BridgeLogicRestrictedAssetDeposited) {
	if f.log.IsDebug() {
		f.log.Debug("Found AssetDeposited event",
			logging.String("bridge-address", f.collateralBridge.HexAddress()),
			logging.String("user-ethereum-address", event.UserAddress.Hex()),
			logging.String("user-fury-address", hex.EncodeToString(event.FuryPublicKey[:])),
			logging.String("asset-id", event.AssetSource.Hex()),
		)
	}
}
			...
			Erc20: &vgproto.ERC20Event{
				...
				Action: &vgproto.ERC20Event_Deposit{
					Deposit: &vgproto.ERC20Deposit{
						...
						TargetPartyId:         hex.EncodeToString(event.FuryPublicKey[:]),
			...
			Erc20: &vgproto.ERC20Event{
				...
				Action: &vgproto.ERC20Event_AssetList{
					AssetList: &vgproto.ERC20AssetList{
						FuryAssetId: hex.EncodeToString(event.FuryAssetId[:]),
			...
			Erc20: &vgproto.ERC20Event{
				...
				Action: &vgproto.ERC20Event_AssetDelist{
					AssetDelist: &vgproto.ERC20AssetDelist{
						FuryAssetId: event.AssetSource.Hex(),
					},
				},
			},
		},
			...
			StakingEvent: &vgproto.StakingEvent{
				...
				Action: &vgproto.StakingEvent_StakeDeposited{
					StakeDeposited: &vgproto.StakeDeposited{
						...
						FuryPublicKey:   hex.EncodeToString(event.FuryPublicKey[:]),
						Amount:          event.Amount.String(),
