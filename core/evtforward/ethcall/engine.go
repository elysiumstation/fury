...
//go:generate go run github.com/golang/mock/mockgen -destination mocks/forwarder_mock.go -package mocks github.com/elysiumstation/fury/core/evtforward/ethcall Forwarder
...
func (e *Engine) OnTick(ctx context.Context, furyTime time.Time) {
	...
	e.log.Info("tick",
		logging.Time("furyTime", furyTime),
...
func makeChainEvent(res Result, specID string, block blockish) *commandspb.ChainEvent {
	ce := commandspb.ChainEvent{
		...
		Event: &commandspb.ChainEvent_ContractCall{
			ContractCall: &fury.EthContractCallEvent{
				SpecId:      specID,
