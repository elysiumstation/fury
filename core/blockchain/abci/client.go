...
func (c *Client) Subscribe(ctx context.Context, fn func(tmctypes.ResultEvent) error, queries ...string) error {
	...
	for _, query := range queries {
		...
		// For subscription we use "fury" as the client name but it's ignored by the implementation.
		// 10 is the channel capacity which is absolutely arbitraty.
		out, err := c.tmclt.Subscribe(ctx, "fury", q.String(), 10)
		if err != nil {
			return err
		}
	...
	defer c.tmclt.UnsubscribeAll(context.Background(), "fury")
