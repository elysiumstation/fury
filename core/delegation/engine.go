...
// reconcile checks if there is a mismatch between the amount associated with FURY by a party and the amount nominated by this party. If a mismatch is found it is auto-adjusted.
...
func (e *Engine) getValidatorData() []*types.ValidatorData {
	...
	for _, vn := range validatorNodes {
		validatorData[vn] = &types.ValidatorData{
			NodeID:            vn,
			PubKey:            e.topology.Get(vn).FuryPubKey,
