...
func (sv *StateVariable) CalculationFinished(eventID string, result statevar.StateVariableResult, err error) {
	...
	// save our result and send the result to fury to be updated by other nodes.
	...
	svp := &commandspb.StateVariableProposal{
		Proposal: &furypb.StateValueProposal{
			StateVarId: sv.ID,
			EventId:    sv.eventID,
			Kvb:        kvb,
		},
	...
	if sv.log.GetLevel() <= logging.DebugLevel {
		sv.log.Debug("result calculated and sent to fury", logging.String("validator", sv.top.SelfNodeID()), logging.String("state-var", sv.ID), logging.String("event-id", eventID))
	}
}
...
func (sv *StateVariable) bundleReceived(ctx context.Context, node, eventID string, bundle *statevar.KeyValueBundle, rng *rand.Rand, validatorVotesRequired num.Decimal) {
	...
	if !sv.top.IsValidatorFuryPubKey(node) {
