...
import (
	...
	"github.com/elysiumstation/fury/core/collateral/mocks"
...
func TestCheckPointLoadingWithAlias(t *testing.T) {
	...
	ab := []*checkpoint.AssetBalance{
		{Party: "*", Asset: "FURY", Balance: "1000"},
		{Party: "*ACCOUNT_TYPE_GLOBAL_REWARD", Asset: "FURY", Balance: "2000"},
	}
	...
	acc, err := e.GetGlobalRewardAccount("FURY")
	require.NoError(t, err)
	require.Equal(t, "3000", acc.Balance.String())

	_, err = e.GetPartyGeneralAccount("*ACCOUNT_TYPE_GLOBAL_REWARD", "FURY")
	require.Error(t, err)
}
