...
func (kvb KeyValueBundle) ToProto() []*fury.KeyValueBundle {
	res := make([]*fury.KeyValueBundle, 0, len(kvb.KVT))
	for _, kvt := range kvb.KVT {
		res = append(res, &fury.KeyValueBundle{
			Key:       kvt.Key,
...
func KeyValueBundleFromProto(protoKVT []*fury.KeyValueBundle) (*KeyValueBundle, error) {
...
func ValueFromProto(val *fury.StateVarValue) (value, error) {
	if val == nil {
	...
	case *fury.StateVarValue_MatrixVal:
