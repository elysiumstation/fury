...
func (dm DecimalMatrix) ToProto() *fury.StateVarValue {
	rows := make([]*fury.VectorValue, 0, len(dm.Val))
	for _, fvi := range dm.Val {
		fviAsString := make([]string, 0, len(fvi))
		for _, dv := range fvi {
			fviAsString = append(fviAsString, dv.String())
		}
		rows = append(rows, &fury.VectorValue{
			Value: fviAsString,
		})
	}
	return &fury.StateVarValue{
		Value: &fury.StateVarValue_MatrixVal{
			MatrixVal: &fury.MatrixValue{
				Value: rows,
			},
		},
	}
