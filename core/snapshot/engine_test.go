...
import (
	...
	"github.com/elysiumstation/fury/core/snapshot/mocks"
	"github.com/elysiumstation/fury/core/types"
	tmocks "github.com/elysiumstation/fury/core/types/mocks"
...
func getTestEngine(t *testing.T) *tstEngine {
	...
	ctx = furyctx.WithChainID(ctx, "chain-id")
	...
	ctx = furyctx.WithTraceID(furyctx.WithBlockHeight(ctx, 1), "0xDEADBEEF")
	return &tstEngine{
		ctx:    ctx,
		cfunc:  cfunc,
		Engine: eng,
...
func TestUpdateInterval(t *testing.T) {
	...
	ctx := furyctx.WithChainID(context.Background(), "chain-id")
	ctx = furyctx.WithTraceID(furyctx.WithBlockHeight(ctx, 10), "0xDEADBEEF")
	...
	e.time.EXPECT().GetTimeNow().Times(1).Return(time.Now())
	ctx = furyctx.WithChainID(context.Background(), "chain-id")
	ctx = furyctx.WithTraceID(furyctx.WithBlockHeight(ctx, 15), "0xDEADBEEF")
	b, error = e.Snapshot(ctx)
	require.NotNil(t, b)
	require.NoError(t, error)
	...
	e.time.EXPECT().GetTimeNow().Times(1).Return(time.Now())
	ctx = furyctx.WithChainID(context.Background(), "chain-id")
	ctx = furyctx.WithTraceID(furyctx.WithBlockHeight(ctx, 30), "0xDEADBEEF")
	b, error = e.Snapshot(ctx)
	require.NotNil(t, b)
	require.NoError(t, error)
	...
	for i := 0; i < 5; i++ {
		ctx = furyctx.WithChainID(context.Background(), "chain-id")
		ctx = furyctx.WithTraceID(furyctx.WithBlockHeight(ctx, 31+int64(i)), "0xDEADBEEF")
		e.time.EXPECT().GetTimeNow().Times(1).Return(time.Now())
		b, error := e.Snapshot(ctx)
		require.NotNil(t, b)
		require.NoError(t, error)
