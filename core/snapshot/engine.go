...
import (
	...
	furyctx "github.com/elysiumstation/fury/libs/context"
	...
	snappb "github.com/elysiumstation/fury/protos/fury/snapshot/v1"
...
//go:generate go run github.com/golang/mock/mockgen -destination mocks/time_mock.go -package mocks github.com/elysiumstation/fury/core/snapshot TimeService
type TimeService interface {
	GetTimeNow() time.Time
	SetTimeNow(context.Context, time.Time)
}

//go:generate go run github.com/golang/mock/mockgen -destination mocks/stats_mock.go -package mocks github.com/elysiumstation/fury/core/snapshot StatsService
...
func New(ctx context.Context, furypath paths.Paths, conf Config, log *logging.Logger, tm TimeService, stats StatsService) (*Engine, error) {
	...
	dbPath, err := conf.validate(furypath)
	if err != nil {
		return nil, err
	}
...
func (e *Engine) applySnap(ctx context.Context) error {
	...
	ctx = furyctx.WithTraceID(furyctx.WithBlockHeight(ctx, int64(e.app.Height)), e.app.Block)
	ctx = furyctx.WithChainID(ctx, e.app.ChainID)
...
func (e *Engine) snapshotNow(ctx context.Context, saveAsync bool) ([]byte, error) {
	...
	appUpdate := false
	height, err := furyctx.BlockHeightFromContext(ctx)
	if err != nil {
		e.avlLock.Unlock()
		return nil, err
	}
	...
	_, block := furyctx.TraceIDFromContext(ctx)
	...
	cid, err := furyctx.ChainIDFromContext(ctx)
	if err != nil {
		e.avlLock.Unlock()
		return nil, err
	}
