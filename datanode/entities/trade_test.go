...
func TestProtoFromTrade(t *testing.T) {
	...
	assert.Equal(t, furyTime.UnixNano(), p.Timestamp)
	...
	assert.Equal(t, fury.Side_SIDE_BUY, p.Aggressor)
	assert.Equal(t, buyOrderIDString, p.BuyOrder)
	assert.Equal(t, sellOrderIDString, p.SellOrder)
	assert.Equal(t, fury.Trade_TYPE_NETWORK_CLOSE_OUT_GOOD, p.Type)
...
func TestTradeFromProto(t *testing.T) {
	...
	testFuryTime := time.Now()
	trade, err := entities.TradeFromProto(&tradeEventProto, generateTxHash(), testFuryTime, 5)
	if err != nil {
		t.Fatalf("failed to convert proto to trade:%s", err)
	}

	assert.Equal(t, testFuryTime.Add(5*time.Microsecond), trade.SyntheticTime)
	assert.Equal(t, testFuryTime, trade.FuryTime)
