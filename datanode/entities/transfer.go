...
type Transfer struct {
	...
	DispatchMetric      *fury.DispatchMetric
...
func (t *Transfer) ToProto(ctx context.Context, accountSource AccountSource) (*eventspb.Transfer, error) {
	...
	case Recurring:
		...
		if t.DispatchMetricAsset != nil {
			recurringTransfer.DispatchStrategy = &fury.DispatchStrategy{
...
func TransferFromProto(ctx context.Context, t *eventspb.Transfer, txHash TxHash, furyTime time.Time, accountSource AccountSource) (*Transfer, error) {
	fromAcc := Account{
		ID:       "",
		...
		TxHash:   txHash,
		FuryTime: time.Unix(0, t.Timestamp),
	}
