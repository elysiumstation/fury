...
type MarketData struct {
	...
	// A synthetic time created which is the sum of fury_time + (seq num * Microsecond)
	SyntheticTime time.Time
	// Transaction which caused this update
	TxHash TxHash
	// Fury Block time at which the data was received from Fury Node
	FuryTime time.Time
...
func (md MarketData) ToProto() *types.MarketData {
	result := types.MarketData{
		...
		Timestamp:                 md.FuryTime.UnixNano(),
