...
// time. FuryTime and Balance will always be set. The others will be nil unless when
...
// - furyTime is assumed to be first
...
func AggregatedBalanceScan(fields []AccountField, rows interface {
	...
	for rows.Next() {
		...
		bal.FuryTime, ok = values[0].(time.Time)
		if !ok {
...
func (balance *AggregatedBalance) ToProto() *v2.AggregatedBalance {
	...
	return &v2.AggregatedBalance{
		Timestamp:   balance.FuryTime.UnixNano(),
