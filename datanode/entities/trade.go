...
func (t Trade) ToProto() *fury.Trade {
	return &fury.Trade{
		Id:        t.ID.String(),
...
func TradeFromProto(t *fury.Trade, txHash TxHash, furyTime time.Time, sequenceNumber uint64) (*Trade, error) {
	syntheticTime := furyTime.Add(time.Duration(sequenceNumber) * time.Microsecond)
