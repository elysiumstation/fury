...
func (lp LiquidityProvision) ToProto() *fury.LiquidityProvision {
	sells := make([]*fury.LiquidityOrderReference, 0, len(lp.Sells))
	buys := make([]*fury.LiquidityOrderReference, 0, len(lp.Buys))
	...
	return &fury.LiquidityProvision{
		Id:               lp.ID.String(),
		...
		Status:           fury.LiquidityProvision_Status(lp.Status),
...
func (lp LiquidityProvision) Key() LiquidityProvisionKey {
	return LiquidityProvisionKey{lp.ID, lp.FuryTime}
}
