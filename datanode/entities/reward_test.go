...
func TestRewardFromProto(t *testing.T) {
	...
	furyTime := entities.NanosToPostgresTimestamp(now.UnixNano())

	reward, err := entities.RewardFromProto(pbReward, generateTxHash(), furyTime, 1)
	...
	fmt.Printf("%v - %v\n", now, reward.FuryTime)
	assert.True(t, furyTime.Equal(reward.FuryTime))
