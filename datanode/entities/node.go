...
func NodeFromValidatorUpdateEvent(evt eventspb.ValidatorUpdate, txHash TxHash, furyTime time.Time) (Node, ValidatorUpdateAux, error) {
	return Node{
			...
			PubKey:          FuryPublicKey(evt.FuryPubKey),
			...
			FuryPubKeyIndex: evt.FuryPubKeyIndex,
			TxHash:          txHash,
		}, nil
}
...
func (rs *RankingScore) ToProto() *fury.RankingScore {
	return &fury.RankingScore{
		StakeScore:       rs.StakeScore.String(),
		PerformanceScore: rs.PerformanceScore.String(),
		PreviousStatus:   fury.ValidatorNodeStatus(rs.PreviousStatus),
		Status:           fury.ValidatorNodeStatus(rs.Status),
...
func (rs *RewardScore) ToProto() *fury.RewardScore {
	return &fury.RewardScore{
		...
		ValidatorStatus:   fury.ValidatorNodeStatus(rs.ValidatorNodeStatus),
	}
}

func NodeFromProto(node *fury.Node, txHash TxHash, furyTime time.Time) (Node, error) {
	...
	return Node{
		ID:                NodeID(node.Id),
		PubKey:            FuryPublicKey(node.PubKey),
...
func (node *Node) ToProto() *fury.Node {
	protoDelegations := make([]*fury.Delegation, len(node.Delegations))
	for i, delegation := range node.Delegations {
	...
	res := &fury.Node{
		...
		Status:            fury.NodeStatus(node.Status),
...
func (ed *EpochData) UnmarshalJSON(b []byte) error {
	ed.EpochData = &fury.EpochData{}
	return protojson.Unmarshal(b, ed)
}

func (n *NodeSet) ToProto() *fury.NodeSet {
	return &fury.NodeSet{
		Total:    n.Total,
		Demoted:  n.Demoted,
		Promoted: n.Promoted,
...
func (n *NodeData) ToProto() *fury.NodeData {
	return &fury.NodeData{
