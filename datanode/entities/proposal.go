...
func (p Proposal) ToProto() *fury.Proposal {
	...
	var reason *fury.ProposalError
	if p.Reason != ProposalErrorUnspecified {
		reason = ptr.From(fury.ProposalError(p.Reason))
	}
	...
	pp := fury.Proposal{
		...
		State:                                  fury.Proposal_State(p.State),
...
func ProposalFromProto(pp *fury.Proposal, txHash TxHash) (Proposal, error) {
	var err error
...
func (pt *ProposalRationale) UnmarshalJSON(b []byte) error {
	pt.ProposalRationale = &fury.ProposalRationale{}
...
func (pt *ProposalTerms) UnmarshalJSON(b []byte) error {
	pt.ProposalTerms = &fury.ProposalTerms{}
	return protojson.Unmarshal(b, pt)
}
