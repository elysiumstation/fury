...
type Vote struct {
	...
	TxHash                      TxHash
	FuryTime                    time.Time // Time of last vote update
}

func (v Vote) ToProto() *fury.Vote {
	return &fury.Vote{
		PartyId:                     v.PartyID.String(),
		ProposalId:                  v.ProposalID.String(),
		Value:                       fury.Vote_Value(v.Value),
...
func VoteFromProto(pv *fury.Vote, txHash TxHash) (Vote, error) {
