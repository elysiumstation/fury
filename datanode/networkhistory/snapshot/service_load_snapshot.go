...
func (b *Service) RollbackToSegment(ctx context.Context, log LoadLog, rollbackToSegment loadableSegment) error {
	...
	for _, meta := range dbMeta.TableNameToMetaData {
		if meta.Hypertable {
			result, err := tx.Exec(ctx, fmt.Sprintf("delete from %s where fury_time > $1", meta.Name), rollbackToBlock.FuryTime)
			if err != nil {
...
func (b *Service) loadSegment(ctx context.Context, furyDbConn *pgx.Conn, loadLog LoadLog, segment loadableSegment, zipDir string,
	...
	startTime := time.Now()
	rowsCopied, err := copyDataIntoDatabase(ctx, furyDbConn, reader, zipDir, dbMetaData, historyTableLastTimestamps)
	if err != nil {
...
func getLastPartitionColumnEntryForHistoryTable(ctx context.Context, furyDbConn *pgxpool.Conn, historyTableMetaData TableMetadata) (time.Time, error) {
	...
	rows, err := furyDbConn.Query(ctx, timeSelect)
	if err != nil {
...
func executeAllSql(ctx context.Context, furyDbConn sqlstore.Connection, loadLog LoadLog, allSql []string) error {
	for _, sql := range allSql {
		loadLog.Infof("executing sql: %s", sql)
		_, err := furyDbConn.Exec(ctx, sql)
		if err != nil {
...
func dropHistoryTableIndexes(ctx context.Context, furyDbConn sqlstore.Connection, loadLog LoadLog,
	dbMetadata DatabaseMetadata,
) ([]IndexInfo, error) {
	var indexes []IndexInfo

	rows, err := furyDbConn.Query(ctx, `select tablename, Indexname, Indexdef from pg_indexes where schemaname ='public' order by tablename`)
	if err != nil {
	...
	for _, index := range indexes {
		_, err = furyDbConn.Exec(ctx, fmt.Sprintf("DROP INDEX %s", index.Indexname))
...
func createIndexes(ctx context.Context, furyDbConn sqlstore.Connection,
	loadLog LoadLog, indexes []IndexInfo,
) error {
	for _, index := range indexes {
		loadLog.Infof("creating index %s", index.Indexname)
		_, err := furyDbConn.Exec(ctx, index.Indexdef)
		if err != nil {
...
func updateContinuousAggregateDataFromHeight(ctx context.Context, conn *pgxpool.Pool, height int64) error {
	...
	for _, cagg := range dbMetaData.ContinuousAggregatesMetaData {
		...
		fromTime := fromBlock.FuryTime.Truncate(cagg.BucketInterval)
...
func UpdateContinuousAggregateDataFromHighWaterMark(ctx context.Context, conn *pgxpool.Pool, toHeight int64) error {
	...
	for _, cagg := range dbMetaData.ContinuousAggregatesMetaData {
		...
		toTime := toBlock.FuryTime.Truncate(cagg.BucketInterval)
