...
import (
	...
	"github.com/elysiumstation/fury/libs/fs"
	vio "github.com/elysiumstation/fury/libs/io"
...
func historyCopySQL(dbMetaData DatabaseMetadata, segment interface{ GetFromHeight() int64 }) []TableCopySql {
	...
	for _, tableName := range tablesNames {
		...
		if dbMetaData.TableNameToMetaData[tableName].Hypertable {
			...
			hyperTableCopySQL := fmt.Sprintf(`copy (select * from %s where %s >= (SELECT fury_time from blocks where height = %d) order by %s) to STDOUT (FORMAT csv, HEADER)`,
				tableName,
				partitionColumn,
