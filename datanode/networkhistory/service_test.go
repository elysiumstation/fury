...
import (
	...
	config2 "github.com/elysiumstation/fury/datanode/config"
...
func getSnapshotIntervalToHistoryTableDeltaSummary(ctx context.Context,
	...
	for i := 0; i < len(expectedHistorySegmentsFromHeights); i++ {
		...
		whereClause := fmt.Sprintf("Where fury_time >= (SELECT fury_time from blocks where height = %d) and  fury_time <= (SELECT fury_time from blocks where height = %d)",
			fromHeight, toHeight)
