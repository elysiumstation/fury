...
func (m *MarketDepth) Initialise(ctx context.Context) error {
	...
	for _, liveOrder := range liveOrders {
		...
		m.AddOrder(order, liveOrder.FuryTime, liveOrder.SeqNum)
	}
...
func (m *MarketDepth) AddOrder(order *types.Order, furyTime time.Time, sequenceNumber uint64) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	// we truncate the furyTime by microsecond because Postgres only supports microsecond
	...
	seqNum := uint64(furyTime.Truncate(time.Microsecond).UnixNano()) + sequenceNumber
