...
type Ledger struct {
	store             ledgerStore
	transferResponses []*fury.LedgerMovement
	observer          utils.Observer[*fury.LedgerMovement]
}

func NewLedger(store ledgerStore, log *logging.Logger) *Ledger {
	return &Ledger{
		store:    store,
		observer: utils.NewObserver[*fury.LedgerMovement]("ledger", log, 0, 0),
	}
}
...
func (l *Ledger) Flush(ctx context.Context) error {
	...
	l.transferResponses = []*fury.LedgerMovement{}
	return nil
}
...
func (l *Ledger) AddTransferResponse(le *fury.LedgerMovement) {
	l.transferResponses = append(l.transferResponses, le)
}

func (l *Ledger) Observe(ctx context.Context, retries int) (<-chan []*fury.LedgerMovement, uint64) {
	ch, ref := l.observer.Observe(ctx,
		retries,
		func(tr *fury.LedgerMovement) bool {
			return true
		})
	return ch, ref
}
