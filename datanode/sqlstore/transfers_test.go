...
func testTransfersGetTransferToOrFromParty(t *testing.T) {
	...
	sourceTransferProto := &eventspb.Transfer{
		...
		Timestamp:       block.FuryTime.UnixNano(),
		Kind: &eventspb.Transfer_Recurring{Recurring: &eventspb.RecurringTransfer{
			StartEpoch: 10,
			EndEpoch:   nil,
			Factor:     "0.1",
			DispatchStrategy: &fury.DispatchStrategy{
				AssetForMetric: "deadd0d0",
				Markets:        []string{"beefdead", "feebaad"},
				Metric:         fury.DispatchMetric_DISPATCH_METRIC_MARKET_VALUE,
			},
		}},
	}

	transfer, err := entities.TransferFromProto(ctx, sourceTransferProto, generateTxHash(), block.FuryTime, accounts)
	...
	transfer, err = entities.TransferFromProto(ctx, sourceTransferProto2, generateTxHash(), block.FuryTime, accounts)
	assert.NoError(t, err)
	err = transfers.Upsert(ctx, transfer)
	assert.NoError(t, err)
...
func testTransfersGetTransfersByParty(t *testing.T) {
	...
	transfer, _ := entities.TransferFromProto(ctx, sourceTransferProto, generateTxHash(), block.FuryTime, accounts)
	...
	transfer, _ = entities.TransferFromProto(ctx, sourceTransferProto2, generateTxHash(), block.FuryTime, accounts)
	transfers.Upsert(ctx, transfer)
...
func testTransfersGetFromAccountAndGetToAccount(t *testing.T) {
	...
	transfer, _ := entities.TransferFromProto(ctx, sourceTransferProto1, generateTxHash(), block.FuryTime, accounts)
	transfers.Upsert(ctx, transfer)
...
func testTransfersUpdatesInDifferentBlocks(t *testing.T) {
	...
	deliverOn := block.FuryTime.Add(1 * time.Hour)
	...
	block = getTestBlock(t, ctx, block.FuryTime.Add(1*time.Microsecond))
...
func testGetByTxHash(t *testing.T) {
	...
	transfer, _ := entities.TransferFromProto(ctx, sourceTransferProto, txHash, block.FuryTime, accounts)
	transfers.Upsert(ctx, transfer)
...
func getTestBlock(t *testing.T, ctx context.Context, testTime time.Time) entities.Block {
	...
	furyTime := time.UnixMicro(testTime.UnixMicro())
	block := addTestBlockForTime(t, ctx, blocks, furyTime)
	return block
}
...
func getTestAccounts(t *testing.T, ctx context.Context, accounts *sqlstore.Accounts, block entities.Block) (accountFrom entities.Account,
	...
	accountFrom = entities.Account{
		...
		AssetID:  testAssetID,
		Type:     fury.AccountType_ACCOUNT_TYPE_GLOBAL_REWARD,
		FuryTime: block.FuryTime,
	}
	...
	accountTo = entities.Account{
		...
		Type:     fury.AccountType_ACCOUNT_TYPE_GENERAL,
		FuryTime: block.FuryTime,
	}
...
func testTransferPaginationNoPagination(t *testing.T) {
	...
	assert.Equal(t, entities.NewCursor(entities.TransferCursor{
		FuryTime: testTransfers[0].FuryTime,
		ID:       testTransfers[0].ID,
	}.String()).Encode(), pageInfo.StartCursor)
	assert.Equal(t, entities.NewCursor(entities.TransferCursor{
		FuryTime: testTransfers[9].FuryTime,
...
func testTransferPaginationFirst(t *testing.T) {
	...
	assert.Equal(t, entities.NewCursor(entities.TransferCursor{
		FuryTime: testTransfers[2].FuryTime,
...
func testTransferPaginationLast(t *testing.T) {
	...
	assert.False(t, pageInfo.HasNextPage)
	assert.Equal(t, entities.NewCursor(entities.TransferCursor{
		FuryTime: testTransfers[7].FuryTime,
...
func testTransferPaginationFirstAfter(t *testing.T) {
	...
	assert.True(t, pageInfo.HasNextPage)
	assert.Equal(t, entities.NewCursor(entities.TransferCursor{
		FuryTime: testTransfers[3].FuryTime,
		ID:       testTransfers[3].ID,
	}.String()).Encode(), pageInfo.StartCursor)
	assert.Equal(t, entities.NewCursor(entities.TransferCursor{
		FuryTime: testTransfers[5].FuryTime,
...
func testTransferPaginationLastBefore(t *testing.T) {
	...
	assert.True(t, pageInfo.HasNextPage)
	assert.Equal(t, entities.NewCursor(entities.TransferCursor{
		FuryTime: testTransfers[4].FuryTime,
		ID:       testTransfers[4].ID,
	}.String()).Encode(), pageInfo.StartCursor)
	assert.Equal(t, entities.NewCursor(entities.TransferCursor{
		FuryTime: testTransfers[6].FuryTime,
...
func addTransfers(ctx context.Context, t *testing.T, bs *sqlstore.Blocks, transferStore *sqlstore.Transfers) []entities.Transfer {
	...
	block := addTestBlockForTime(t, ctx, bs, furyTime)
	accounts := sqlstore.NewAccounts(connectionSource)
	accountFrom, accountTo := getTestAccounts(t, ctx, accounts, block)
