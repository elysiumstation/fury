...
foreign_tx_hash, foreign_block_height, foreign_block_time, log_index, ethereum_address, tx_hash, fury_time`
)
...
set
	...
	if _, err := s.Connection.Exec(ctx, query, stake.ID, stake.StakeLinkingType, stake.EthereumTimestamp, stake.PartyID, stake.Amount,
		...
		stake.EthereumAddress, stake.TxHash, stake.FuryTime); err != nil {
		return err
	}
