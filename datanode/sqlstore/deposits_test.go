...
func testAddDepositForNewBlock(t *testing.T) {
	...
	deposit, err := entities.DepositFromProto(depositProto, generateTxHash(), block.FuryTime)
...
func testUpdateDepositForBlockIfExists(t *testing.T) {
	...
	deposit.Status = entities.DepositStatus(fury.Deposit_STATUS_FINALIZED)
	...
	err = pgxscan.Get(ctx, conn, &status, `select status from deposits where id = $1 and fury_time = $2`, deposit.ID, deposit.FuryTime)
	assert.NoError(t, err)
...
func testInsertDepositUpdatesIfNewBlock(t *testing.T) {
	...
	block = source.getNextBlock(t, ctx)
	depositProto.Status = fury.Deposit_STATUS_FINALIZED
	deposit, err = entities.DepositFromProto(depositProto, generateTxHash(), block.FuryTime)
	require.NoError(t, err, "Converting market proto to database entity")
...
func testDepositsGetByParty(t *testing.T) {
	...
	deposit, err := entities.DepositFromProto(depositProto1, generateTxHash(), block.FuryTime)
	require.NoError(t, err, "Converting market proto to database entity")
	...
	block = source.getNextBlock(t, ctx)
	depositProto1.Status = fury.Deposit_STATUS_FINALIZED
	deposit, err = entities.DepositFromProto(depositProto1, generateTxHash(), block.FuryTime)
	require.NoError(t, err, "Converting market proto to database entity")
	...
	block = source.getNextBlock(t, ctx)
	deposit, err = entities.DepositFromProto(depositProto2, generateTxHash(), block.FuryTime)
	require.NoError(t, err, "Converting market proto to database entity")
	...
	block = source.getNextBlock(t, ctx)
	deposit, err = entities.DepositFromProto(depositProto2, generateTxHash(), block.FuryTime)
	depositProto2.Status = fury.Deposit_STATUS_FINALIZED
	require.NoError(t, err, "Converting market proto to database entity")
...
func getTestDeposit(id, party, asset, amount, txHash string, ts int64) *fury.Deposit {
	return &fury.Deposit{
		Id:                id,
		Status:            fury.Deposit_STATUS_OPEN,
		PartyId:           party,
		Asset:             asset,
		Amount:            amount,
		TxHash:            txHash,
...
func addDeposits(ctx context.Context, t *testing.T, bs *sqlstore.Blocks, ds *sqlstore.Deposits) []entities.Deposit {
	...
	for i := 0; i < 10; i++ {
		...
		depositProto := getTestDeposit(fmt.Sprintf("deadbeef%02d", i+1), testID, testID,
			strconv.FormatInt(amount, 10), helpers.GenerateID(), furyTime.UnixNano())
		deposit, err := entities.DepositFromProto(depositProto, generateTxHash(), furyTime)
...
func testDepositsPaginationBetweenDatesNoPagination(t *testing.T) {
	...
	startDate := testDeposits[3].FuryTime
	endDate := testDeposits[8].FuryTime
...
func testDepositsPaginationBetweenDatesFirst(t *testing.T) {
	...
	startDate := testDeposits[2].FuryTime
	endDate := testDeposits[8].FuryTime
