...
func testInsertNewInCurrentBlock(t *testing.T) {
	...
	data, err := entities.LiquidityProvisionFromProto(lpProto[0], generateTxHash(), block.FuryTime)
...
func testGetLPByReferenceAndParty(t *testing.T) {
	...
	for _, lpp := range lpProto {
		...
		data, err := entities.LiquidityProvisionFromProto(lpp, generateTxHash(), block.FuryTime)
		require.NoError(t, err)
		assert.NoError(t, lp.Upsert(ctx, data))
		err = lp.Flush(ctx)
		require.NoError(t, err)
...
func getTestLiquidityProvision(live bool) []*fury.LiquidityProvision {
	testres := []*fury.LiquidityProvision{
		{
			Id:               "deadbeef",
			PartyId:          "deadbaad",
			...
			Buys:             nil,
			Version:          0,
			Status:           fury.LiquidityProvision_STATUS_ACTIVE,
			Reference:        "TEST1",
		},
		{
			Id:               "0d15ea5e",
		...
		},
		{
			...
			Buys:             nil,
			Version:          0,
			Status:           fury.LiquidityProvision_STATUS_PENDING,
			Reference:        "TEST4",
		},
	}
			...
			[]*fury.LiquidityProvision{
				{
					...
					Buys:             nil,
					Version:          0,
					Status:           fury.LiquidityProvision_STATUS_STOPPED,
					Reference:        "TEST5",
				},
				{
					Id:               "deadc0ef",
					...
					Buys:             nil,
					Version:          0,
					Status:           fury.LiquidityProvision_STATUS_CANCELLED,
					Reference:        "TEST6",
				},
			}...)
	}
...
func testLiquidityProvisionPaginationNoPagination(t *testing.T) {
	...
	assert.Equal(t, entities.NewCursor(entities.LiquidityProvisionCursor{
		FuryTime: testLps[0].FuryTime,
		ID:       testLps[0].ID,
	}.String()).Encode(), pageInfo.StartCursor)
	assert.Equal(t, entities.NewCursor(entities.LiquidityProvisionCursor{
		FuryTime: testLps[9].FuryTime,
		ID:       testLps[9].ID,
...
func testLiquidityProvisionPaginationFirstAfter(t *testing.T) {
	...
	assert.True(t, pageInfo.HasNextPage)
	assert.Equal(t, entities.NewCursor(entities.LiquidityProvisionCursor{
		FuryTime: testLps[3].FuryTime,
		ID:       testLps[3].ID,
	}.String()).Encode(), pageInfo.StartCursor)
	assert.Equal(t, entities.NewCursor(entities.LiquidityProvisionCursor{
		FuryTime: testLps[5].FuryTime,
		ID:       testLps[5].ID,
	}.String()).Encode(), pageInfo.EndCursor)
}
...
func testLiquidityProvisionPaginationLastBefore(t *testing.T) {
	...
	assert.True(t, pageInfo.HasNextPage)
	assert.Equal(t, entities.NewCursor(entities.LiquidityProvisionCursor{
		FuryTime: testLps[4].FuryTime,
		ID:       testLps[4].ID,
	}.String()).Encode(), pageInfo.StartCursor)
	assert.Equal(t, entities.NewCursor(entities.LiquidityProvisionCursor{
		FuryTime: testLps[6].FuryTime,
		ID:       testLps[6].ID,
	}.String()).Encode(), pageInfo.EndCursor)
}
...
func addLiquidityProvisions(ctx context.Context, t *testing.T, bs *sqlstore.Blocks, lpstore *sqlstore.LiquidityProvision) []entities.LiquidityProvision {
	...
	for i := 0; i < 10; i++ {
		...
		lp := &fury.LiquidityProvision{
			Id:               fmt.Sprintf("deadbeef%02d", i+1),
			PartyId:          "deadbaad",
			CreatedAt:        furyTime.UnixNano(),
			UpdatedAt:        furyTime.UnixNano(),
			MarketId:         "cafed00d",
			CommitmentAmount: "100000",
			Fee:              "0.3",
			Sells:            nil,
		...
		withdrawal, err := entities.LiquidityProvisionFromProto(lp, generateTxHash(), furyTime)
