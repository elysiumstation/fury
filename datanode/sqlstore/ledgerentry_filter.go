...
func getUniqueAccountTypes(accountTypes []fury.AccountType) []fury.AccountType {
	accountTypesList := []fury.AccountType{}
	accountTypesMap := map[fury.AccountType]struct{}{}
...
func transferTypeFilterToDBQuery(transferTypeFilter []entities.LedgerMovementType, args *[]interface{}) string {
	...
	if len(transferTypeFilter) > 0 {
		...
		for v := range transferTypesMap {
			_, ok := fury.TransferType_name[int32(v)]
			if !ok {
				continue
			}
