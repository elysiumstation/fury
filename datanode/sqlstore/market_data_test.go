...
func shouldInsertAValidMarketDataRecord(t *testing.T) {
	...
	err = md.Add(&entities.MarketData{
		...
		PriceMonitoringBounds: []*fury.PriceMonitoringBounds{
			{
				MinValidPrice: "1",
				MaxValidPrice: "2",
				Trigger: &fury.PriceMonitoringTrigger{
					Horizon:          100,
					Probability:      "0.5",
...
func getLatestMarketData(t *testing.T) {
	...
	want := entities.MarketData{
		...
		LiquidityProviderFeeShares: []*fury.LiquidityProviderFeeShare{
			{
		...
		FuryTime: time.Date(2022, 2, 11, 10, 5, 41, 0, time.UTC),
	}
...
func setupMarketData(t *testing.T, ctx context.Context) (*sqlstore.MarketData, error) {
	...
	for {
		...
		if _, alreadyAdded := addedBlocksAt[marketData.FuryTime.UnixNano()]; !alreadyAdded {
			// Postgres only stores timestamps in microsecond resolution
			block := entities.Block{
				FuryTime: marketData.FuryTime,
				Height:   2,
				Hash:     hash,
			}
			...
			require.NoError(t, err)
			addedBlocksAt[marketData.FuryTime.UnixNano()] = struct{}{}
		}
...
func mustParsePriceMonitoringBounds(t *testing.T, value string) []*fury.PriceMonitoringBounds {
	t.Helper()
	if strings.ToLower(value) == "null" {
		return nil
	}

	var bounds []*fury.PriceMonitoringBounds
...
func mustParseLiquidity(t *testing.T, value string) []*fury.LiquidityProviderFeeShare {
	t.Helper()
	if strings.ToLower(value) == "null" {
		return nil
	}

	var liquidity []*fury.LiquidityProviderFeeShare
...
func csvToMarketData(t *testing.T, line []string, seqNum int) *entities.MarketData {
	...
	furyTime := mustParseTimestamp(t, line[csvColumnFuryTime])
	syntheticTime := furyTime.Add(time.Duration(seqNum) * time.Microsecond)
