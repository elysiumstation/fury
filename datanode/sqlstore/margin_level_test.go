...
func testInsertMarginLevels(t *testing.T) {
	...
	marginLevel, err := entities.MarginLevelsFromProto(ctx, marginLevelProto, accountStore, generateTxHash(), block.FuryTime)
...
func getMarginLevelProto() *fury.MarginLevels {
	return getMarginLevelWithMaintenanceProto("1000", "deadbeef", "deadbeef", time.Now().UnixNano())
}

func getMarginLevelWithMaintenanceProto(maintenanceMargin, partyID, marketID string, timestamp int64) *fury.MarginLevels {
...
func testGetMarginLevelsByPartyID(t *testing.T) {
	...
	marginLevel1, err := entities.MarginLevelsFromProto(ctx, ml1, accountStore, generateTxHash(), block.FuryTime)
	require.NoError(t, err, "Converting margin levels proto to database entity")

	marginLevel2, err := entities.MarginLevelsFromProto(ctx, ml2, accountStore, generateTxHash(), block.FuryTime)
	require.NoError(t, err, "Converting margin levels proto to database entity")
	...
	block = blockSource.getNextBlock(t, ctx)
	marginLevel3, err := entities.MarginLevelsFromProto(ctx, ml3, accountStore, generateTxHash(), block.FuryTime)
	require.NoError(t, err, "Converting margin levels proto to database entity")

	marginLevel4, err := entities.MarginLevelsFromProto(ctx, ml4, accountStore, generateTxHash(), block.FuryTime)
	require.NoError(t, err, "Converting margin levels proto to database entity")
	...
	want1.FuryTime = want1.FuryTime.Truncate(time.Microsecond)

	want2 := marginLevel4
	want2.Timestamp = want2.Timestamp.Truncate(time.Microsecond)
	want2.FuryTime = want2.FuryTime.Truncate(time.Microsecond)
...
func populateMarginLevelPaginationTestData(t *testing.T, ctx context.Context) (*sqlstore.MarginLevels, map[int]entities.Block, map[int]entities.MarginLevels) {
	...
	for i, ml := range margins {
		block := blockSource.getNextBlock(t, ctx)
		mlProto := getMarginLevelWithMaintenanceProto(ml.maintenanceMargin, ml.partyID, ml.marketID, block.FuryTime.UnixNano())
		mlEntity, err := entities.MarginLevelsFromProto(ctx, mlProto, accountStore, generateTxHash(), block.FuryTime)
		require.NoError(t, err, "Converting margin levels proto to database entity")
...
func testGetMarginLevelsByIDPaginationWithMarketNoCursor(t *testing.T) {
	...
	assert.Equal(t, wantMarginLevels, got)
	wantStartCursor := entities.MarginCursor{
		FuryTime:  blocks[2].FuryTime,
		AccountID: marginLevels[2].AccountID,
	}
	wantEndCursor := entities.MarginCursor{
		FuryTime:  blocks[13].FuryTime,
