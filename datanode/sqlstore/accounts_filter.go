...
func filterAccountsQuery(af entities.AccountFilter, includeFuryTime bool) (string, []interface{}, error) {
	var args []interface{}
	var err error

	query := `SELECT id, party_id, asset_id, market_id, type, tx_hash FROM ACCOUNTS `
	if includeFuryTime {
		query = `SELECT id, party_id, asset_id, market_id, type, tx_hash, fury_time FROM ACCOUNTS `
	}
...
func currentAccountBalancesQuery() string {
	return `SELECT ACCOUNTS.id, ACCOUNTS.party_id, ACCOUNTS.asset_id, ACCOUNTS.market_id, ACCOUNTS.type,
			current_balances.balance, current_balances.tx_hash, current_balances.fury_time
			FROM ACCOUNTS JOIN current_balances ON ACCOUNTS.id = current_balances.account_id `
}

func accountBalancesQuery() string {
	return `SELECT ACCOUNTS.id, ACCOUNTS.party_id, ACCOUNTS.asset_id, ACCOUNTS.market_id, ACCOUNTS.type,
			balances.balance, balances.tx_hash, balances.fury_time
