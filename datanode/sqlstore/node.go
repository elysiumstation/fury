...
func selectNodeQuery() string {
	...
	this_epoch AS (
		SELECT nodes.id AS node_id,
			COALESCE(SUM(current_delegations.amount) FILTER (WHERE current_delegations.party_id = nodes.fury_pub_key), 0) AS staked_by_operator,
			COALESCE(SUM(current_delegations.amount) FILTER (WHERE current_delegations.party_id != nodes.fury_pub_key), 0) AS staked_by_delegates,
...
func (store *Node) UpsertNode(ctx context.Context, node *entities.Node) error {
	...
	_, err := store.Connection.Exec(ctx, `
		...
		SET
			fury_pub_key = EXCLUDED.fury_pub_key,
...
func (store *Node) AddNodeAnnouncedEvent(ctx context.Context, nodeID string, furytime time.Time, aux *entities.ValidatorUpdateAux) error {
	...
	_, err := store.Connection.Exec(ctx, `
		...
		VALUES
			($1, $2, $3, $4, $5)
		ON CONFLICT (node_id, epoch_seq, fury_time) DO UPDATE SET
...
func (store *Node) UpdatePublicKey(ctx context.Context, kr *entities.KeyRotation) error {
	...
	_, err := store.Connection.Exec(ctx, `UPDATE nodes SET fury_pub_key = $1 WHERE id = $2`, kr.NewPubKey, kr.NodeID)
...
func (store *Node) GetByTxHash(ctx context.Context, txHash entities.TxHash) ([]entities.NodeBasic, error) {
	...
	query := `SELECT id, fury_pub_key, tendermint_pub_key, ethereum_address, name, location, 
