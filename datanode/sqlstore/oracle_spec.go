...
const (
	sqlOracleSpecColumns = `id, created_at, updated_at, signers, filters, status, tx_hash, fury_time`
)
...
set
	...
	if _, err := os.Connection.Exec(ctx, query, dataSourceSpec.ID, dataSourceSpec.CreatedAt, dataSourceSpec.UpdatedAt, signers,
		filters, dataSourceSpec.Status, dataSourceSpec.TxHash, dataSourceSpec.FuryTime); err != nil {
		return err
	}
...
func (os *OracleSpec) GetSpecByID(ctx context.Context, specID string) (*entities.OracleSpec, error) {
	...
	query := fmt.Sprintf(`%s
where id = $1
order by id, fury_time desc`, getOracleSpecsQuery())
