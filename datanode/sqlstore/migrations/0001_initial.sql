...
create table blocks
(
    fury_time     TIMESTAMP WITH TIME ZONE NOT NULL PRIMARY KEY,
    height        BIGINT                   NOT NULL,
    hash          BYTEA                    NOT NULL
);
select create_hypertable('blocks', 'fury_time', chunk_time_interval => INTERVAL '1 day');
...
$$
BEGIN
    Insert into last_block (fury_time, height, hash) VALUES(NEW.fury_time, NEW.height, NEW.hash) on conflict(onerow_check) do update
    set
        fury_time=EXCLUDED.fury_time,
...
(
    ...
    fury_time           TIMESTAMP WITH TIME ZONE NOT NULL,
    PRIMARY KEY (id, fury_time)
);

CREATE VIEW assets_current AS (
  SELECT DISTINCT ON (id) * FROM assets ORDER BY id, fury_time DESC
);
...
(
    ...
    tx_hash   BYTEA NOT NULL,
    fury_time TIMESTAMP WITH TIME ZONE
);
...
(
    ...
    PRIMARY KEY(fury_time, account_id) INCLUDE (balance)
);

select create_hypertable('balances', 'fury_time', chunk_time_interval => INTERVAL '1 day');
create index on balances (account_id, fury_time) INCLUDE(balance);
...
$$
    BEGIN
    INSERT INTO current_balances(account_id, tx_hash, fury_time, balance) VALUES(NEW.account_id, NEW.tx_hash, NEW.fury_time, NEW.balance)
...
SELECT account_id, time_bucket('1 hour', fury_time) AS bucket,
       last(balance, fury_time) AS balance,
       last(tx_hash, fury_time) AS tx_hash,
       last(fury_time, fury_time) AS fury_time
FROM balances
...
SELECT
    ...
    conflated_balances.fury_time,
    conflated_balances.balance
FROM conflated_balances
WHERE conflated_balances.fury_time < (SELECT coalesce(min(balances.fury_time), 'infinity') FROM balances));
...
CREATE INDEX ON ledger (account_from_id, fury_time DESC);
CREATE INDEX ON ledger (account_to_id, fury_time DESC);
CREATE INDEX ON ledger (type, fury_time DESC);
...
CREATE TABLE orders (
    ...
    PRIMARY KEY(fury_time, seq_num)
);

SELECT create_hypertable('orders', 'fury_time', chunk_time_interval => INTERVAL '1 day');

create index on orders (id, fury_time desc, seq_num desc);
create index on orders (created_at desc, id, fury_time desc, seq_num desc);
create index on orders (market_id, created_at desc, id, fury_time desc, seq_num desc);
create index on orders (party_id, created_at desc, id, fury_time desc, seq_num desc);
create index on orders (reference, created_at desc, id, fury_time desc, seq_num desc);
...
CREATE INDEX ON orders_live (market_id, fury_time DESC);
CREATE INDEX ON orders_live (party_id, fury_time DESC);
CREATE INDEX ON orders_live (reference, fury_time DESC);
...
BEGIN
    ...
    -- As per https://github.com/elysiumstation/specs-internal/blob/master/protocol/0024-OSTA-order_status.md
...
-- As per https://github.com/elysiumstation/fury/issues/8149, only LIMIT type (1) orders with status active (1) and parked (8)
-- and time_in_force != IOC (3) and time_in_force != FOK (4) are considered live.
    ...
    THEN
        ...
        VALUES(new.id, new.market_id, new.party_id, new.side, new.price,
               ...
               new.tx_hash, new.fury_time, new.seq_num, new.post_only, new.reduce_only);
    END IF;
...
CREATE OR REPLACE VIEW orders_current_versions AS (
    SELECT DISTINCT ON (id, version) * FROM orders ORDER BY id, version DESC, fury_time DESC
);
...
FROM orders
ORDER BY orders.created_at DESC, orders.id, orders.fury_time DESC, orders.seq_num DESC;

CREATE OR REPLACE VIEW orders_current_desc_by_market
AS
SELECT DISTINCT ON (orders.created_at, orders.market_id, orders.id) *
FROM orders
ORDER BY orders.created_at DESC, orders.market_id, orders.id, orders.fury_time DESC, orders.seq_num DESC;

CREATE OR REPLACE VIEW orders_current_desc_by_party
AS
SELECT DISTINCT ON (orders.created_at, orders.party_id, orders.id) *
FROM orders
ORDER BY orders.created_at DESC, orders.party_id, orders.id, orders.fury_time DESC, orders.seq_num DESC;

CREATE OR REPLACE VIEW orders_current_desc_by_reference
AS
SELECT DISTINCT ON (orders.created_at, orders.reference, orders.id) *
FROM orders
ORDER BY orders.created_at DESC, orders.reference, orders.id, orders.fury_time DESC, orders.seq_num DESC;
...
SELECT market_id,  fury_time as period_start,
       first(price, synthetic_time) AS open,
       last(price, synthetic_time) AS close,
       max(price) AS high,
       min(price) AS low,
...
create index on market_data (market, fury_time);
...
$$
BEGIN
    INSERT INTO current_market_data(synthetic_time,tx_hash,fury_time,seq_num,market,mark_price,best_bid_price,best_bid_volume,
    ...
    VALUES(NEW.synthetic_time, NEW.tx_hash, NEW.fury_time, NEW.seq_num, NEW.market,
    ...
    ON CONFLICT(market) DO UPDATE SET
                                      synthetic_time=EXCLUDED.synthetic_time,tx_hash=EXCLUDED.tx_hash,fury_time=EXCLUDED.fury_time,seq_num=EXCLUDED.seq_num,market=EXCLUDED.market,mark_price=EXCLUDED.mark_price,
...
CREATE TABLE IF NOT EXISTS nodes_announced (
  ...
  tx_hash               BYTEA NOT NULL,
  fury_time             TIMESTAMP WITH TIME ZONE NOT NULL,
  PRIMARY KEY(node_id, epoch_seq, fury_time)
);
...
SELECT create_hypertable('rewards', 'fury_time', chunk_time_interval => INTERVAL '1 day', migrate_data => true);
...
select create_hypertable('delegations', 'fury_time', chunk_time_interval => INTERVAL '1 day');
...
$$
BEGIN
    INSERT INTO delegations_current(party_id,node_id,epoch_id,amount,tx_hash,fury_time,seq_num)
    VALUES(NEW.party_id,NEW.node_id,NEW.epoch_id,NEW.amount,NEW.tx_hash,NEW.fury_time,NEW.seq_num)
...
SELECT create_hypertable('markets', 'fury_time', chunk_time_interval => INTERVAL '1 day');
...
create table if not exists deposits (
    ...
    tx_hash  bytea not null,
    fury_time timestamp with time zone not null,
    primary key (id, party_id, fury_time)
);
CREATE INDEX ON deposits(party_id);

select create_hypertable('deposits', 'fury_time', chunk_time_interval => INTERVAL '1 day');
...
CREATE VIEW deposits_current AS (
    ...
    SELECT DISTINCT ON (id, party_id) * FROM deposits ORDER BY id, party_id, fury_time DESC
);
...
select create_hypertable('withdrawals', 'fury_time', chunk_time_interval => INTERVAL '1 day');
...
CREATE VIEW withdrawals_current AS (
    ...
    SELECT DISTINCT ON (id, party_id) * FROM withdrawals ORDER BY id, party_id, fury_time DESC
);
...
CREATE TABLE votes(
  ...
  PRIMARY KEY (proposal_id, party_id, fury_time)
);

CREATE INDEX ON votes(party_id);

CREATE VIEW votes_current AS (
  SELECT DISTINCT ON (proposal_id, party_id) * FROM votes ORDER BY proposal_id, party_id, fury_time DESC
);
...
create table if not exists margin_levels (
    ...
    tx_hash BYTEA NOT NULL,
    fury_time timestamp with time zone not null,
    PRIMARY KEY(fury_time, account_id)
);

select create_hypertable('margin_levels', 'fury_time', chunk_time_interval => INTERVAL '1 day');
...
INSERT INTO current_margin_levels(account_id,
                                  ...
                                  tx_hash,
                                  fury_time) VALUES(NEW.account_id,
                                                    NEW.timestamp,
...
CREATE MATERIALIZED VIEW conflated_margin_levels
            WITH (timescaledb.continuous, timescaledb.materialized_only = true) AS
SELECT account_id, time_bucket('1 minute', fury_time) AS bucket,
       last(maintenance_margin, fury_time) AS maintenance_margin,
       last(search_level, fury_time) AS search_level,
       last(initial_margin, fury_time) AS initial_margin,
       last(collateral_release_level, fury_time) AS collateral_release_level,
       last(timestamp, fury_time) AS timestamp,
       last(tx_hash, fury_time) AS tx_hash,
       last(fury_time, fury_time) AS fury_time
FROM margin_levels
GROUP BY account_id, bucket WITH NO DATA;
...
SELECT conflated_margin_levels.account_id,
       ...
       conflated_margin_levels.fury_time
FROM conflated_margin_levels
WHERE conflated_margin_levels.fury_time < (SELECT coalesce(min(margin_levels.fury_time), 'infinity') FROM margin_levels));
...
create table if not exists risk_factors (
    ...
    tx_hash bytea not null,
    fury_time timestamp with time zone not null,
    primary key (market_id, fury_time)
);

CREATE VIEW risk_factors_current AS (
    SELECT DISTINCT ON (market_id) * FROM risk_factors ORDER BY market_id, fury_time DESC
);
...
$$
BEGIN
    INSERT INTO network_parameters_current(key, value, tx_hash, fury_time)
    VALUES(NEW.key, NEW.value, NEW.tx_hash, NEW.fury_time)
    ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value, tx_hash=EXCLUDED.tx_hash, fury_time=EXCLUDED.fury_time;
    RETURN NULL;
END;
$$;
-- +goose StatementEnd
...
SELECT create_hypertable('checkpoints', 'fury_time', chunk_time_interval => INTERVAL '1 day');
...
CREATE TABLE positions(
  ...
  primary key (fury_time, party_id, market_id)
);

select create_hypertable('positions', 'fury_time', chunk_time_interval => INTERVAL '1 day');

CREATE INDEX ON positions(party_id, market_id, fury_time);

CREATE MATERIALIZED VIEW conflated_positions
            WITH (timescaledb.continuous, timescaledb.materialized_only = true) AS
SELECT market_id, party_id, time_bucket('1 hour', fury_time) AS bucket,
 last(open_volume, fury_time) AS open_volume,
 last(realised_pnl, fury_time) AS realised_pnl,
 last(unrealised_pnl, fury_time) AS unrealised_pnl,
 last(average_entry_price, fury_time) AS average_entry_price,
 last(average_entry_market_price, fury_time) AS average_entry_market_price,
 last(loss, fury_time) AS loss,
 last(adjustment, fury_time) AS adjustment,
 last(tx_hash, fury_time) AS tx_hash,
 last(fury_time, fury_time) AS fury_time
FROM positions
...
SELECT
    ...
    conflated_positions.fury_time
FROM conflated_positions
WHERE conflated_positions.fury_time < (SELECT coalesce(min(positions.fury_time), 'infinity') FROM positions));
...
$$
BEGIN
    INSERT INTO positions_current(market_id,party_id,open_volume,realised_pnl,unrealised_pnl,average_entry_price,average_entry_market_price,loss,adjustment,tx_hash,fury_time,pending_open_volume,pending_realised_pnl,pending_unrealised_pnl,pending_average_entry_price,pending_average_entry_market_price, loss_socialisation_amount, distressed_status)
    VALUES(NEW.market_id,NEW.party_id,NEW.open_volume,NEW.realised_pnl,NEW.unrealised_pnl,NEW.average_entry_price,NEW.average_entry_market_price,NEW.loss,NEW.adjustment,NEW.tx_hash,NEW.fury_time,NEW.pending_open_volume,NEW.pending_realised_pnl,NEW.pending_unrealised_pnl,NEW.pending_average_entry_price,NEW.pending_average_entry_market_price, NEW.loss_socialisation_amount, NEW.distressed_status)
...
$$
BEGIN
    INSERT INTO oracle_data_current(signers,data,matched_spec_ids,broadcast_at,tx_hash,fury_time,seq_num)
    VALUES(NEW.signers,NEW.data,NEW.matched_spec_ids,NEW.broadcast_at,NEW.tx_hash,NEW.fury_time,NEW.seq_num)
...
select create_hypertable('liquidity_provisions', 'fury_time', chunk_time_interval => INTERVAL '1 day');
...
-- Active (1), Undeployed (5), Pending (6)
    ...
    THEN
        INSERT INTO live_liquidity_provisions(id, party_id, created_at, updated_at,
                                              market_id, commitment_amount, fee, sells, buys, version, status, reference, tx_hash, fury_time)
        VALUES(NEW.id, NEW.party_id, NEW.created_at, NEW.updated_at,
               NEW.market_id, NEW.commitment_amount, NEW.fee, NEW.sells,
               NEW.buys, NEW.version, NEW.status, NEW.reference, NEW.tx_hash, NEW.fury_time);
    END IF;
...
CREATE VIEW transfers_current AS ( SELECT DISTINCT ON (id, from_account_id, to_account_id) * FROM transfers ORDER BY id, from_account_id, to_account_id, fury_time DESC);
...
$$
BEGIN
    INSERT INTO stake_linking_current (id, stake_linking_type, ethereum_timestamp, party_id, amount, stake_linking_status, finalized_at, foreign_tx_hash, foreign_block_height, foreign_block_time, log_index, ethereum_address, tx_hash, fury_time)
    VALUES (NEW.id,
...
CREATE TABLE IF NOT EXISTS protocol_upgrade_proposals(
    ...
    fury_release_tag TEXT NOT NULL,
    approvers TEXT[] NOT NULL,
    status protocol_upgrade_proposal_status NOT NULL,
    tx_hash bytea not null,
    fury_time TIMESTAMP WITH TIME ZONE NOT NULL,
    PRIMARY KEY(fury_time, upgrade_block_height, fury_release_tag)
);

CREATE VIEW protocol_upgrade_proposals_current AS (
    SELECT DISTINCT ON (upgrade_block_height, fury_release_tag) *
      FROM protocol_upgrade_proposals
  ORDER BY upgrade_block_height, fury_release_tag, fury_time DESC);

CREATE TABLE IF NOT EXISTS core_snapshots(
    block_height BIGINT NOT NULL,
    block_hash TEXT null,
    fury_core_version TEXT null,
    tx_hash bytea not null,
    fury_time TIMESTAMP WITH TIME ZONE NOT NULL,
    PRIMARY KEY(fury_time, block_height)
);
...
DROP INDEX IF EXISTS positions_party_id_market_id_fury_time_idx;
