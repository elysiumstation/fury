...
func (ps *Proposals) Add(ctx context.Context, p entities.Proposal) error {
	...
	_, err := ps.Connection.Exec(ctx,
		`INSERT INTO proposals(
		 ...
		 ON CONFLICT (id, fury_time) DO UPDATE SET
			...
			;
		 `,
		p.ID, p.Reference, p.PartyID, p.State, p.Terms, p.Rationale, p.Reason, p.ErrorDetails, p.ProposalTime, p.FuryTime, p.RequiredMajority, p.RequiredParticipation, p.RequiredLPMajority, p.RequiredLPParticipation, p.TxHash)
	return err
}
...
func (ps *Proposals) Get(ctx context.Context,
	...
	// In order to do this, we need to construct a union of proposals where state = open, order by fury_time
	// and state != open, order by fury_time
