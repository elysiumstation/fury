...
func (as *Accounts) Add(ctx context.Context, a *entities.Account) error {
	...
	err := as.Connection.QueryRow(ctx,
		`INSERT INTO accounts(id, party_id, asset_id, market_id, type, tx_hash, fury_time)
...
func (as *Accounts) GetByID(ctx context.Context, accountID entities.AccountID) (entities.Account, error) {
	...
	if err := pgxscan.Get(ctx, as.Connection, &a,
		`SELECT id, party_id, asset_id, market_id, type, tx_hash, fury_time
...
func (as *Accounts) GetByTxHash(ctx context.Context, txHash entities.TxHash) ([]entities.Account, error) {
	...
	err := pgxscan.Select(
		...
		as.Connection,
		&accounts,
		`SELECT id, party_id, asset_id, market_id, type, tx_hash, fury_time FROM accounts WHERE tx_hash=$1`,
		txHash,
	)
	return accounts, err
}
...
func (as *Accounts) Obtain(ctx context.Context, a *entities.Account) error {
	...
	insertQuery := `INSERT INTO accounts(id, party_id, asset_id, market_id, type, tx_hash, fury_time)
                           VALUES ($1, $2, $3, $4, $5, $6, $7)
                           ON CONFLICT (party_id, asset_id, market_id, type) DO NOTHING`

	selectQuery := `SELECT id, party_id, asset_id, market_id, type, tx_hash, fury_time
	                FROM accounts
	                WHERE party_id=$1 AND asset_id=$2 AND market_id=$3 AND type=$4`

	batch := pgx.Batch{}

	batch.Queue(insertQuery, accountID, a.PartyID, a.AssetID, a.MarketID, a.Type, a.TxHash, a.FuryTime)
