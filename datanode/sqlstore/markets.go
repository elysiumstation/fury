...
const (
	sqlMarketsColumns = `id, tx_hash, fury_time, instrument_id, tradable_instrument, decimal_places,
...
on conflict (id, fury_time) do update
set
	...
	if _, err := m.Connection.Exec(ctx, query, market.ID, market.TxHash, market.FuryTime, market.InstrumentID, market.TradableInstrument, market.DecimalPlaces,
