...
func getByIDShouldReturnTheRequestedMarketIfItExists(t *testing.T) {
	...
	assert.Equal(t, market.FuryTime, marketFromDB.FuryTime)
...
func getByTxHashReturnsMatchingMarkets(t *testing.T) {
	...
	assert.Equal(t, market.FuryTime, foundMarkets[0].FuryTime)
...
func getAllPagedShouldNotIncludeRejectedMarkets(t *testing.T) {
	...
	assert.Equal(t, market.FuryTime, markets[0].FuryTime)
...
func shouldInsertAValidMarketRecord(t *testing.T) {
	...
	market, err := entities.NewMarketFromProto(marketProto, generateTxHash(), block.FuryTime)
...
func shouldUpdateAValidMarketRecord(t *testing.T) {
	...
	t.Run("should insert a valid market record to the database", func(t *testing.T) {
		...
		err = pgxscan.Get(ctx, conn, &got, `select * from markets where id = $1 and fury_time = $2`, market.ID, market.FuryTime)
		assert.NoError(t, err)
	...
	t.Run("should add the updated market record to the database if the block number has changed", func(t *testing.T) {
		...
		market, err := entities.NewMarketFromProto(newMarketProto, generateTxHash(), newBlock.FuryTime)
		require.NoError(t, err, "Converting market proto to database entity")
		...
		err = pgxscan.Get(ctx, conn, &gotFirstBlock, `select * from markets where id = $1 and fury_time = $2`, market.ID, block.FuryTime)
		assert.NoError(t, err)
		assert.Equal(t, "TEST_INSTRUMENT", market.InstrumentID)

		assert.Equal(t, marketProto.TradableInstrument, gotFirstBlock.TradableInstrument.ToProto())

		err = pgxscan.Get(ctx, conn, &gotSecondBlock, `select * from markets where id = $1 and fury_time = $2`, market.ID, newBlock.FuryTime)
		assert.NoError(t, err)
		assert.Equal(t, "TEST_INSTRUMENT", market.InstrumentID)
...
func getTestMarket() *fury.Market {
	return &fury.Market{
		Id: "DEADBEEF",
		TradableInstrument: &fury.TradableInstrument{
			Instrument: &fury.Instrument{
				Id:   "TEST_INSTRUMENT",
				Code: "TEST",
				Name: "Test Instrument",
				Metadata: &fury.InstrumentMetadata{
					Tags: []string{"AAA", "BBB"},
				},
				Product: &fury.Instrument_Future{
					Future: &fury.Future{
						SettlementAsset: "Test Asset",
						QuoteName:       "Test Quote",
						DataSourceSpecForSettlementData: &fury.DataSourceSpec{
							Id:        "",
							CreatedAt: 0,
							UpdatedAt: 0,
							Data: fury.NewDataSourceDefinition(
								fury.DataSourceDefinitionTypeExt,
							).SetOracleConfig(
								&fury.DataSourceSpecConfiguration{
									Signers: nil,
									Filters: nil,
								},
							),
						...
						DataSourceSpecForTradingTermination: &fury.DataSourceSpec{
							Id:        "",
							CreatedAt: 0,
							UpdatedAt: 0,
							Data: fury.NewDataSourceDefinition(
						...
						DataSourceSpecBinding: &fury.DataSourceSpecToFutureBinding{
			...
			MarginCalculator: &fury.MarginCalculator{
			...
			RiskModel: &fury.TradableInstrument_SimpleRiskModel{
				SimpleRiskModel: &fury.SimpleRiskModel{
					Params: &fury.SimpleModelParams{
						FactorLong:           0,
						FactorShort:          0,
						MaxMoveUp:            0,
						MinMoveDown:          0,
		...
		DecimalPlaces: 16,
		Fees: &fury.Fees{
			Factors: &fury.FeeFactors{
				MakerFee:          "",
				InfrastructureFee: "",
				LiquidityFee:      "",
			},
		},
		OpeningAuction: &fury.AuctionDuration{
			Duration: 0,
			Volume:   0,
		},
		PriceMonitoringSettings: &fury.PriceMonitoringSettings{
			Parameters: &fury.PriceMonitoringParameters{
				Triggers: []*fury.PriceMonitoringTrigger{
					{
						Horizon:          0,
						Probability:      "",
						AuctionExtension: 0,
		...
		LiquidityMonitoringParameters: &fury.LiquidityMonitoringParameters{
			TargetStakeParameters: &fury.TargetStakeParameters{
				TimeWindow:    0,
				ScalingFactor: 0,
			},
			TriggeringRatio:  "0",
		...
		TradingMode: fury.Market_TRADING_MODE_CONTINUOUS,
		State:       fury.Market_STATE_ACTIVE,
		MarketTimestamps: &fury.MarketTimestamps{
			Proposed: 0,
			Pending:  0,
			Open:     0,
			Close:    0,
...
func populateTestMarkets(ctx context.Context, t *testing.T, bs *sqlstore.Blocks, md *sqlstore.Markets, blockTimes map[string]time.Time) {
	...
	for _, market := range markets {
		...
		market.FuryTime = block.FuryTime
		blockTimes[market.ID.String()] = block.FuryTime
		err := md.Upsert(ctx, &market)
		require.NoError(t, err)
	}
}
...
func testCursorPaginationReturnsAllMarkets(t *testing.T) {
	...
	wantStartCursor := entities.NewCursor(
		entities.MarketCursor{
			FuryTime: blockTimes["02a16077"],
			ID:       "02a16077",
		}.String(),
	).Encode()
	wantEndCursor := entities.NewCursor(
		entities.MarketCursor{
			FuryTime: blockTimes["fb1528a5"],
			ID:       "fb1528a5",
		}.String(),
	).Encode()
	assert.Equal(t, entities.PageInfo{
...
func testCursorPaginationReturnsPageTraversingForward(t *testing.T) {
	...
	wantStartCursor := entities.NewCursor(
		entities.MarketCursor{
			FuryTime: blockTimes["7a797e0e"],
			ID:       "7a797e0e",
		}.String(),
	).Encode()
	wantEndCursor := entities.NewCursor(
		entities.MarketCursor{
			FuryTime: blockTimes["b7c84b8e"],
			ID:       "b7c84b8e",
		}.String(),
	).Encode()
	assert.Equal(t, entities.PageInfo{
