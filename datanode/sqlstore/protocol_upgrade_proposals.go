...
var pupOrdering = TableOrdering{
	...
	ColumnOrdering{Name: "fury_release_tag", Sorting: ASC},
}
...
func (ps *ProtocolUpgradeProposals) Add(ctx context.Context, p entities.ProtocolUpgradeProposal) error {
	...
	_, err := ps.Connection.Exec(ctx,
		`INSERT INTO protocol_upgrade_proposals(
			...
			tx_hash)
		 VALUES ($1,  $2,  $3,  $4,  $5,  $6)
		 ON CONFLICT (fury_time, upgrade_block_height, fury_release_tag) DO UPDATE SET
			approvers = EXCLUDED.approvers,
			status = EXCLUDED.status,
			tx_hash = EXCLUDED.tx_hash;
		`,
		p.UpgradeBlockHeight, p.FuryReleaseTag, p.Approvers, p.Status, p.FuryTime, p.TxHash)
	return err
}
...
func (ps *ProtocolUpgradeProposals) GetByTxHash(
	...
	query := `SELECT upgrade_block_height, fury_release_tag, approvers, status, fury_time, tx_hash
