...
const (
	sqlDepositsColumns = `id, status, party_id, asset, amount, foreign_tx_hash,
		credited_timestamp, created_timestamp, tx_hash, fury_time`

	depositsFilterDateColumn = "fury_time"
)
...
on conflict (id, party_id, fury_time) do update
set
	...
	if _, err := d.Connection.Exec(ctx, query, deposit.ID, deposit.Status, deposit.PartyID, deposit.Asset, deposit.Amount,
		deposit.ForeignTxHash, deposit.CreditedTimestamp, deposit.CreatedTimestamp, deposit.TxHash, deposit.FuryTime); err != nil {
...
func (d *Deposits) GetByID(ctx context.Context, depositID string) (entities.Deposit, error) {
	...
	query := `select id, status, party_id, asset, amount, foreign_tx_hash, credited_timestamp, created_timestamp, tx_hash, fury_time
		from deposits_current
		where id = $1
		order by id, party_id, fury_time desc`
