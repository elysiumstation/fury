...
func insertTestData(t *testing.T, ctx context.Context, tradeStore *sqlstore.Trades) []entities.Trade {
	...
	var seqNum uint64
	furyTime := block1.FuryTime
	for _, proto := range protos {
		if seqNum == 3 {
			seqNum = 0
			furyTime = block2.FuryTime
		}
		trade, err := entities.TradeFromProto(&proto, generateTxHash(), furyTime, seqNum)
		if err != nil {
...
func populateTestTrades(ctx context.Context, t *testing.T, bs *sqlstore.Blocks, ts *sqlstore.Trades, blockTimes map[string]time.Time) {
	...
	for _, td := range trades {
		...
		trade.SyntheticTime = block.FuryTime
		trade.FuryTime = block.FuryTime
		blockTimes[trade.ID.String()] = block.FuryTime
		err := ts.Add(&trade)
