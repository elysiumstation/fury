...
func (n *Node) consumeUpdate(ctx context.Context, event ValidatorUpdateEvent) error {
	node, aux, err := entities.NodeFromValidatorUpdateEvent(event.ValidatorUpdate(), entities.TxHash(event.TxHash()), n.furyTime)
	if err != nil {
	...
	return errors.Wrap(n.store.AddNodeAnnouncedEvent(ctx, node.ID.String(), node.FuryTime, &aux), "inserting node to SQL store failed")
}

func (n *Node) consumeRankingScore(ctx context.Context, event ValidatorRankingScoreEvent) error {
	ranking, aux, err := entities.RankingScoreFromRankingEvent(event.ValidatorRankingEvent(), entities.TxHash(event.TxHash()), n.furyTime)
	if err != nil {
...
func (n *Node) consumeRewardScore(ctx context.Context, event ValidatorRewardScoreEvent) error {
	reward, aux, err := entities.RewardScoreFromScoreEvent(event.ValidatorScoreEvent(), entities.TxHash(event.TxHash()), n.furyTime)
	if err != nil {
...
func (n *Node) consumeKeyRotation(ctx context.Context, event KeyRotationEvent) error {
	...
	record, err := entities.KeyRotationFromProto(&keyRotation, entities.TxHash(event.TxHash()), n.furyTime)
	if err != nil {
...
func (n *Node) consumeEthereumKeyRotation(ctx context.Context, event EthereumKeyRotationEvent) error {
	...
	record, err := entities.EthereumKeyRotationFromProto(&keyRotation, entities.TxHash(event.TxHash()), n.furyTime,
		event.Sequence())
	if err != nil {
