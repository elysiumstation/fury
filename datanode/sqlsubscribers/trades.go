...
func (ts *TradeSubscriber) consume(te TradeEvent) error {
	trade := te.Trade()
	return errors.Wrap(ts.addTrade(&trade, entities.TxHash(te.TxHash()), ts.furyTime, te.Sequence()), "failed to consume trade")
}

func (ts *TradeSubscriber) addTrade(t *types.Trade, txHash entities.TxHash, furyTime time.Time, blockSeqNumber uint64) error {
	trade, err := entities.TradeFromProto(t, txHash, furyTime, blockSeqNumber)
	if err != nil {
