...
type MarketDepthService interface {
	AddOrder(order *types.Order, furyTime time.Time, sequenceNumber uint64)
	PublishAtEndOfBlock()
}
...
func (os *Order) expired(ctx context.Context, eo ExpiredOrdersEvent, seqNum uint64) error {
	...
	for _, o := range orders {
		...
		o.SeqNum = seqNum
		o.UpdatedAt = os.furyTime
		o.FuryTime = os.furyTime
		o.TxHash = txHash
		...
		os.depthService.AddOrder(torder, os.furyTime, seqNum)
