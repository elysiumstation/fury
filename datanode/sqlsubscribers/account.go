...
func (as *Account) consume(ctx context.Context, evt AccountEvent) error {
	...
	acc, err := as.obtainAccountWithProto(ctx, &protoAcc, evt.TxHash(), as.furyTime)
	if err != nil {
...
func (as *Account) obtainAccountWithProto(ctx context.Context, va *fury.Account, txHash string, furyTime time.Time) (entities.Account, error) {
