...
func (rs *Reward) consume(ctx context.Context, event RewardPayoutEvent) error {
	...
	reward, err := entities.RewardFromProto(protoRewardPayoutEvent, entities.TxHash(event.TxHash()), rs.furyTime, event.Sequence())
	if err != nil {
		return errors.Wrap(err, "unable to parse reward")
	}

	if reward.FuryTime != rs.furyTime {
		return errors.Errorf("reward timestamp does not match current FuryTime. Reward:%v",
