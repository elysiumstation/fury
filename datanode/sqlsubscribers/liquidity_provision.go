...
type LiquidityProvisionEvent interface {
	events.Event
	LiquidityProvision() *fury.LiquidityProvision
}
...
func (lp *LiquidityProvision) consume(ctx context.Context, event LiquidityProvisionEvent) error {
	entity, err := entities.LiquidityProvisionFromProto(event.LiquidityProvision(), entities.TxHash(event.TxHash()), lp.furyTime)
	if err != nil {
