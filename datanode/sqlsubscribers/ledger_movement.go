...
type Ledger interface {
	...
	AddTransferResponse(*fury.LedgerMovement)
	Flush(ctx context.Context) error
}

type TransferResponseEvent interface {
	events.Event
	LedgerMovements() []*fury.LedgerMovement
}
...
func (t *TransferResponse) consume(ctx context.Context, e TransferResponseEvent) error {
	...
	for _, tr := range e.LedgerMovements() {
		...
		for _, vle := range tr.Entries {
			if err := t.addLedgerEntry(ctx, vle, e.TxHash(), t.furyTime); err != nil {
...
func (t *TransferResponse) addLedgerEntry(ctx context.Context, vle *fury.LedgerEntry, txHash string, furyTime time.Time) error {
	fromAcc, err := t.obtainAccountWithAccountDetails(ctx, vle.FromAccount, txHash, furyTime)
	if err != nil {
		return errors.Wrap(err, "obtaining 'from' account")
	}

	toAcc, err := t.obtainAccountWithAccountDetails(ctx, vle.ToAccount, txHash, furyTime)
	if err != nil {
...
// Parse the fury account ID; if that account already exists in the db, fetch it; else create it.
func (t *TransferResponse) obtainAccountWithAccountDetails(ctx context.Context, ad *fury.AccountDetails, txHash string, furyTime time.Time) (entities.Account, error) {
