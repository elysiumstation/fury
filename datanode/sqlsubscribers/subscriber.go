...
//go:generate go run github.com/golang/mock/mockgen -destination mocks/mocks.go -package mocks github.com/elysiumstation/fury/datanode/sqlsubscribers RiskFactorStore,TransferStore,WithdrawalStore,LiquidityProvisionStore,KeyRotationStore,OracleSpecStore,DepositStore,StakeLinkingStore,MarketDataStore,PositionStore,OracleDataStore,MarginLevelsStore,NotaryStore,NodeStore,MarketsStore,MarketSvc

type subscriber struct {
	furyTime time.Time
}

func (s *subscriber) SetFuryTime(furyTime time.Time) {
	s.furyTime = furyTime
}
