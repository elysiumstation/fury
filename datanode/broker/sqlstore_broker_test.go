...
import (
	...
	eventsv1 "github.com/elysiumstation/fury/protos/fury/events/v1"
...
func TestSqlBrokerSubscriberCallbacks(t *testing.T) {
	s1 := testSQLBrokerSubscriber{
		...
		furyTimeCh: make(chan time.Time),
	...
	// BlockEnd event should cause a flush of subscribers, followed by commit and then an update to subscribers furytime,
	...
	assert.Equal(t, time.Unix(0, beginEvent.BeginBlock().Timestamp), <-s1.furyTimeCh)
	...
	expectedBlock := entities.Block{
		FuryTime: time.Unix(0, beginEvent.BeginBlock().Timestamp).Truncate(time.Microsecond),
		Hash:     hash,
	...
	assert.Equal(t, time.Unix(0, beginEvent.BeginBlock().Timestamp).Truncate(time.Microsecond), <-s1.furyTimeCh)
	assert.Equal(t, true, <-transactionManager.withTransactionCalls)
...
func TestSqlBrokerTimeEventSentToAllSubscribers(t *testing.T) {
	...
	assert.Equal(t, time.Unix(0, blockEvent.BeginBlock().Timestamp).
		Truncate(time.Microsecond), <-s1.furyTimeCh)
	assert.Equal(t, time.Unix(0, blockEvent.BeginBlock().Timestamp).
		Truncate(time.Microsecond), <-s2.furyTimeCh)
}
...
func newTestSQLBrokerSubscriber(eventType events.Type) *testSQLBrokerSubscriber {
	return &testSQLBrokerSubscriber{
		...
		furyTimeCh: make(chan time.Time, 100),
	}
}

func (t *testSQLBrokerSubscriber) SetFuryTime(furyTime time.Time) {
	t.furyTimeCh <- furyTime
}
...
func newBlockEventSource() *blockEventSource {
	return &blockEventSource{
		furyTime:    time.Now().Truncate(time.Millisecond),
		blockHeight: 1,
	}
}
...
func (s *blockEventSource) NextEndBlockEvent() *events.EndBlock {
	...
	s.furyTime = s.furyTime.Add(1 * time.Second)
	s.blockHeight++
