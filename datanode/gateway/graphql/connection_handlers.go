...
func handleCandleConnectionRequest(ctx context.Context, client TradingDataServiceClientV2, market *fury.Market, sinceRaw string, toRaw *string,
	interval fury.Interval, pagination *v2.Pagination,
) (*v2.CandleDataConnection, error) {
	since, err := furytime.Parse(sinceRaw)
	if err != nil {
		return nil, err
	}
...
func toV2IntervalString(interval fury.Interval) (string, error) {
	switch interval {
	case fury.Interval_INTERVAL_BLOCK:
		return "block", nil
	case fury.Interval_INTERVAL_I1M:
		return "1 minute", nil
	case fury.Interval_INTERVAL_I5M:
		return "5 minutes", nil
	case fury.Interval_INTERVAL_I15M:
		return "15 minutes", nil
	case fury.Interval_INTERVAL_I1H:
		return "1 hour", nil
	case fury.Interval_INTERVAL_I6H:
		return "6 hours", nil
	case fury.Interval_INTERVAL_I1D:
		return "1 day", nil
	default:
...
func handleProposalsRequest(ctx context.Context, client TradingDataServiceClientV2, party *types.Party, ref *string, inType *v2.ListGovernanceDataRequest_Type,
	inState *fury.Proposal_State, pagination *v2.Pagination,
