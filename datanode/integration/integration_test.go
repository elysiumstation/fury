...
import (
	...
	"github.com/elysiumstation/fury/cmd/data-node/commands/start"
	"github.com/elysiumstation/fury/datanode/config"
	"github.com/elysiumstation/fury/datanode/config/encoding"
	"github.com/elysiumstation/fury/datanode/utils"
	"github.com/elysiumstation/fury/datanode/utils/databasetest"
...
func TestMain(m *testing.M) {
	...
	furyHome, postgresRuntimePath, err := setupDirs()
	if err != nil {
	...
	go func() {
		defer wg.Done()
		if err := runTestNode(ctx, cfg, furyHome, stopper); err != nil {
			close(stopper)
			cfunc()
...
func runTestNode(ctx context.Context, cfg *config.Config, furyHome string, stopper chan struct{}) error {
	furyPaths := paths.New(furyHome)

	loader, err := config.InitialiseLoader(furyPaths)
	if err != nil {
	...
	configWatcher, err := config.NewWatcher(context.Background(), logger, furyPaths)
	if err != nil {
	...
	if err = cmd.Run(ctx, configWatcher, furyPaths, []string{}); err != nil {
