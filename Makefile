...
spellcheck: ## Run markdown spellcheck container
	@docker run --rm -ti \
		--entrypoint mdspell \
		-v "$(PWD):/src" \
		ghcr.io/elysiumstation/devops-infra/markdownspellcheck:latest \
			--en-gb \
			--ignore-acronyms \
			--ignore-numbers \
			--no-suggestions \
...
proto_check: ## proto: Check committed files match just-generated files
	...
	@files="$$(git diff --name-only protos/fury/ protos/data-node/)" ; \
	if test -n "$$files" ; then \
		echo "Committed files do not match just-generated files: " $$files ; \
		git diff ; \
		exit 1 ; \
...
.PHONY: proto_clean
proto_clean:
	@find protos/fury protos/data-node -name '*.pb.go' -o -name '*.pb.gw.go' \
		| xargs -r rm
