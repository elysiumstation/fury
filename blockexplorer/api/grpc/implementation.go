...
import (
	...
	"github.com/elysiumstation/fury/blockexplorer/entities"
	"github.com/elysiumstation/fury/blockexplorer/store"
	"github.com/elysiumstation/fury/logging"
	pb "github.com/elysiumstation/fury/protos/blockexplorer/api/v1"
...
// errorMap contains a mapping between errors and Fury numeric error codes.
...
// apiError is a helper function to build the Fury specific Error Details that
// can be returned by gRPC API and therefore also REST, GraphQL will be mapped too.
// It takes a standardised grpcCode, a Fury specific apiError, and optionally one
...
func apiError(grpcCode codes.Code, apiError error) error {
	...
	furyCode, found := errorMap[apiError.Error()]
	if found {
		detail.Code = furyCode
	} else {
		detail.Code = errorMap[ErrNotMapped.Error()]
	}
	// Pack the Fury domain specific errorDetails into the status returned by gRPC domain.
