...
import (
	...
	wcrypto "github.com/elysiumstation/fury/wallet/crypto"
...
func CheckTransaction(tx *commandspb.Transaction, chainID string) (*commandspb.InputData, error) {
	...
	} else if !IsFuryPubkey(tx.GetPubKey()) {
		errs.AddForProperty("tx.from.pub_key", ErrShouldBeAValidFuryPubkey)
	}
