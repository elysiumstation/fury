...
func checkUpdateAssetChanges(change *protoTypes.ProposalTerms_UpdateAsset) Errors {
	...
	} else if !IsFuryPubkey(change.UpdateAsset.AssetId) {
		errs.AddForProperty("proposal_submission.terms.change.update_asset.asset_id", ErrShouldBeAValidFuryID)
	}
...
func checkUpdateMarketChanges(change *protoTypes.ProposalTerms_UpdateMarket) Errors {
	...
	} else if !IsFuryPubkey(change.UpdateMarket.MarketId) {
		errs.AddForProperty("proposal_submission.terms.change.update_market.market_id", ErrShouldBeAValidFuryID)
	}
...
func checkUpdateSpotMarketChanges(change *protoTypes.ProposalTerms_UpdateSpotMarket) Errors {
	...
	} else if !IsFuryPubkey(change.UpdateSpotMarket.MarketId) {
		errs.AddForProperty("proposal_submission.terms.change.update_spot_market.market_id", ErrShouldBeAValidFuryID)
	}
...
func checkDataSourceSpec(spec *furypb.DataSourceDefinition, name string, parentProperty string, tryToSettle bool) Errors {
	errs := NewErrors()
	if spec == nil {
	...
	case *furypb.DataSourceDefinition_External:
		...
		for i, key := range signers {
			...
			} else if pubkey := signer.GetSignerPubKey(); pubkey != nil && !crypto.IsValidFuryPubKey(pubkey.Key) {
				errs.AddForProperty(fmt.Sprintf("%s.%s.external.oracle.signers.%d", parentProperty, name, i), ErrIsNotValidFuryPubkey)
...
func isBindingMatchingSpec(spec *furypb.DataSourceDefinition, bindingProperty string) bool {
