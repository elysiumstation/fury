...
func checkDelegateSubmission(cmd *commandspb.DelegateSubmission) Errors {
	...
	} else if !IsFuryPubkey(cmd.NodeId) {
		errs.AddForProperty("delegate_submission.node_id", ErrShouldBeAValidFuryPubkey)
	}
...
func checkUndelegateSubmission(cmd *commandspb.UndelegateSubmission) Errors {
	...
	if len(cmd.NodeId) <= 0 {
		...
		errs.AddForProperty("undelegate_submission.node_id", ErrShouldBeAValidFuryPubkey)
	}
