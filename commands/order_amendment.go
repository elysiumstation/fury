...
func checkOrderAmendment(cmd *commandspb.OrderAmendment) Errors {
	...
	} else if !IsFuryPubkey(cmd.OrderId) {
		errs.AddForProperty("order_amendment.order_id", ErrShouldBeAValidFuryID)
	}

	if len(cmd.MarketId) <= 0 {
		errs.AddForProperty("order_amendment.market_id", ErrIsRequired)
	} else if !IsFuryPubkey(cmd.MarketId) {
		errs.AddForProperty("order_amendment.market_id", ErrShouldBeAValidFuryID)
	}
