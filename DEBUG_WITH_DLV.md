...
- Use "Command Palette" to run `Debug: Open launch.json`. If you didn't already have a `launch.json` file, this will create one with the below default configuration which can be used to debug the current package. Enter the following into `launch.json` (which will be by default created inside `fury/.vscode/`):
        ...
        "configurations": [
            {
                ...
                "request": "attach",
                "mode": "remote",
                "remotePath": "/path/to/fury", // trading-core
                "port": 2345,
                "host": "127.0.0.1",
                "showLog":true,
                "trace":"log"
...
- Now open ideally two terminal windows. In the first one launch `tendermint` as you normally would. To hard-reset the chain and start from scratch use
    ...
    ```bash
    dlv exec /path/to/fury/cmd/fury/fury-dbg --headless --listen=:2345 --log --api-version=2 -- node
    ```
    ...
    2019-10-13T20:37:41+01:00 info layer=debugger launching process with args: [/path/to/fury/cmd/fury/fury-dbg node]
    debugserver-@(#)PROGRAM:LLDB  PROJECT:lldb-1100.0.28..1
     for x86_64.
    Got a connection, launched process /path/to/fury/cmd/fury/fury (pid = 35671).
    ```
...
- Build a debug version of the `godog` test harness: from the root of the `V E G A` core repository
    ...
    ```bash
    DEBUGFURY=yes go test -c ./integration/...
    ```
...
To build the fury binary for debugging, then:

```bash
go test -c -gcflags="ALL=-N -l" ./cmd/fury
```
...
The output will be a binary called either `fury.test` or `integration.test`.
...
The address and port to listen to is arbitrary, but in this particular example, we'll use 9876. In this example we're starting a debug session on the integration test binary. To debug `fury.test`, just replace `integration.test` with `fury.test` (obviously).
