...
import (
	...
	"github.com/elysiumstation/fury/visor/client"
...
func (v *Visor) Run(ctx context.Context) error {
	...
	for {
		...
		c := v.clientFactory.GetClient(
			runConf.Fury.RCP.SocketPath,
			runConf.Fury.RCP.HTTPPath,
		)
	...
	CheckLoop:
		for {
			...
			case <-upgradeTicker.C:
				...
				if err := v.prepareNextUpgradeFolder(ctx, currentReleaseInfo.FuryReleaseTag); err != nil {
