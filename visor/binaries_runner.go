...
func (r *BinariesRunner) runBinary(ctx context.Context, binPath string, args []string) error {
	...
	if r.releaseInfo != nil {
		if err := ensureBinaryVersion(binPath, r.releaseInfo.FuryReleaseTag); err != nil {
			return err
		}
	}
...
func (r *BinariesRunner) prepareFuryArgs(runConf *config.RunConfig, isRestart bool) (Args, error) {
	args := Args(runConf.Fury.Binary.Args)
...
func (r *BinariesRunner) Run(ctx context.Context, runConf *config.RunConfig, isRestart bool) chan error {
	r.log.Debug("Starting Fury binary")

	eg, ctx := errgroup.WithContext(ctx)

	eg.Go(func() error {
		args, err := r.prepareFuryArgs(runConf, isRestart)
		if err != nil {
			return fmt.Errorf("failed to prepare args for Fury binary: %w", err)
		}

		return r.runBinary(ctx, runConf.Fury.Binary.Path, args)
	})
