...
func GetNetworkVersionThroughGRPC(hosts []string) (string, error) {
	...
	// pings fury.xyz to verify whether or not they are connected to the internet.
	// This should help the user with troubleshooting their setup.
	ctx, cancelFn := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancelFn()
	req, err := http.NewRequestWithContext(ctx, http.MethodHead, "https://fury.xyz", nil)
	if err != nil {
	...
	// `fury.xyz`, which suggest the issue comes from the network configuration
