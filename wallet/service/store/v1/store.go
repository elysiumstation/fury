...
import (
	...
	"github.com/elysiumstation/fury/wallet/service/v1"
)
...
func InitialiseStore(furyPaths paths.Paths) (*Store, error) {
	pubRsaKeyFilePath, err := furyPaths.CreateDataPathFor(paths.WalletServicePublicRSAKeyDataFile)
	if err != nil {
		return nil, fmt.Errorf("couldn't get data path for %s: %w", paths.WalletServicePublicRSAKeyDataFile, err)
	}

	privRsaKeyFilePath, err := furyPaths.CreateDataPathFor(paths.WalletServicePrivateRSAKeyDataFile)
	if err != nil {
		return nil, fmt.Errorf("couldn't get data path for %s: %w", paths.WalletServicePrivateRSAKeyDataFile, err)
	}

	configFilePath, err := furyPaths.CreateConfigPathFor(paths.WalletServiceDefaultConfigFile)
	if err != nil {
