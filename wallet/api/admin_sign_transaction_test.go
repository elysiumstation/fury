...
import (
	...
	"github.com/elysiumstation/fury/wallet/api/mocks"
	walletnode "github.com/elysiumstation/fury/wallet/api/node"
	nodemocks "github.com/elysiumstation/fury/wallet/api/node/mocks"
	"github.com/elysiumstation/fury/wallet/api/node/types"
...
func testAdminSigningTransactionWithMalformedTransactionFails(t *testing.T) {
	...
	// then
	assertInvalidParams(t, errorDetails, errors.New("the transaction does not use a valid Fury command: unknown field \"bob\" in fury.wallet.v1.SubmitTransactionRequest"))
	assert.Empty(t, result)
}
...
func testAdminSigningTransactionWithInvalidTransactionFails(t *testing.T) {
	...
	// then
	assertInvalidParams(t, errorDetails, fmt.Errorf("vote_submission.proposal_id (should be a valid fury ID)"))
	assert.Empty(t, result)
}
