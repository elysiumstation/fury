...
func testSendingTransactionWithInvalidParamsFails(t *testing.T) {
	tcs := []struct {
		...
		{
			name: "with transaction as invalid Fury command",
			params: api.ClientSendTransactionParams{
				PublicKey:   vgrand.RandomStr(10),
				SendingMode: "TYPE_SYNC",
				Transaction: map[string]interface{}{
					"type": "not fury command",
				},
			},
			expectedError: errors.New("the transaction does not use a valid Fury command: unknown field \"type\" in fury.wallet.v1.SubmitTransactionRequest"),
		},
	}
