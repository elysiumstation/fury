...
import (
	...
	v1 "github.com/elysiumstation/fury/wallet/network/store/v1"
...
func testNewStoreSucceeds(t *testing.T) {
	...
	s, err := v1.InitialiseStore(furyHome)

	require.NoError(t, err)
	assert.NotNil(t, s)
	vgtest.AssertDirAccess(t, networksHome(t, furyHome))
}
...
func testFileStoreV1SaveNetworkSucceeds(t *testing.T) {
	...
	// then
	require.NoError(t, err)
	vgtest.AssertFileAccess(t, networkPath(t, furyHome, net.Name))

	// confirm that the network name is not saved in the file
	netpath := networkPath(t, furyHome, net.Name)
...
func testFileStoreV1GetNetworkPathSucceeds(t *testing.T) {
	...
	// then
	assert.Equal(t, networkPath(t, furyHome, "test"), returnedPath)
}
...
func testFileStoreV1GetNetworksPathSucceeds(t *testing.T) {
	...
	// then
	assert.Equal(t, networksHome(t, furyHome), returnedPath)
}
...
func initialiseFromPath(t *testing.T, furyHome paths.Paths) *v1.FileStore {
	t.Helper()
	s, err := v1.InitialiseStore(furyHome)
	if err != nil {
...
func networksHome(t *testing.T, furyHome *paths.CustomPaths) string {
	t.Helper()
	return furyHome.ConfigPathFor(paths.WalletServiceNetworksConfigHome)
}

func networkPath(t *testing.T, furyHome *paths.CustomPaths, name string) string {
	t.Helper()
	return filepath.Join(networksHome(t, furyHome), name+".toml")
}
