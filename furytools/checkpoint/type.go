...
import (
	...
	events "github.com/elysiumstation/fury/protos/fury/events/v1"
...
func dummy() *all {
	ae := &checkpoint.AssetEntry{
		Id: "ETH",
		AssetDetails: &fury.AssetDetails{
			Name:     "ETH",
			Symbol:   "ETH",
			Decimals: 5,
			Quantum:  "",
			Source: &fury.AssetDetails_BuiltinAsset{
				BuiltinAsset: &fury.BuiltinAsset{
	...
	prop := &fury.Proposal{
		...
		PartyId:   "deadbeef007",
		State:     fury.Proposal_STATE_ENACTED,
		Timestamp: time.Now().Add(-1 * time.Hour).Unix(),
		Terms: &fury.ProposalTerms{
			...
			Change: &fury.ProposalTerms_NewMarket{
				NewMarket: &fury.NewMarket{
					Changes: &fury.NewMarketConfiguration{
						Instrument: &fury.InstrumentConfiguration{
							Name: "ETH/FOO",
							Code: "bar",
							Product: &fury.InstrumentConfiguration_Future{
								Future: &fury.FutureProduct{ // omitted oracle spec for now
									SettlementAsset: "ETH",
									QuoteName:       "ETH",
								},
							},
						...
						DecimalPlaces: 5,
						PriceMonitoringParameters: &fury.PriceMonitoringParameters{
						...
						RiskParameters: &fury.NewMarketConfiguration_LogNormal{
							LogNormal: &fury.LogNormalRiskModel{
								RiskAversionParameter: 0.1,
								Tau:                   0.2,
								Params: &fury.LogNormalModelParams{
									Mu:    0.3,
									R:     0.3,
									Sigma: 0.3,
								},
	...
	return &all{
		messages: map[string]proto.Message{
			...
			"governance": &checkpoint.Proposals{
				Proposals: []*fury.Proposal{prop},
			},
			"network_parameters": &checkpoint.NetParams{
				Params: []*fury.NetworkParameter{
					{
						Key:   "foo",
						Value: "bar",
					},
			...
			"epoch": &events.EpochEvent{
				Seq:        0,
				Action:     fury.EpochAction_EPOCH_ACTION_START,
